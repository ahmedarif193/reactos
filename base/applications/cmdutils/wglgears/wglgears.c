...
print_resources(const char *phase)
{
   ...
   printf("WGLGEARS_RESOURCES phase=%s gdi=%lu user=%lu\n",
          phase,
...
query_vsync()
{
   ...
   if (GLAD_WGL_EXT_swap_control) {
      if (!wglSwapIntervalEXT(0)) {
         printf("WGLGEARS_VSYNC disable_failed\n");
      }
      interval = wglGetSwapIntervalEXT();
      printf("WGLGEARS_VSYNC control=available interval=%d\n", interval);
   }
   else {
      printf("WGLGEARS_VSYNC control=unavailable\n");
   }
...
main(int argc, char *argv[])
{
   ...
   /* cleanup */
   gladLoaderUnloadGL();
   print_resources("loader_unloaded");
   wglMakeCurrent (NULL, NULL);
   print_resources("context_released");
   wglDeleteContext (hRC);
   print_resources("context_deleted");
   ReleaseDC (hWnd, hDC);
   print_resources("dc_released");
   DestroyWindow (hWnd);
   print_resources("window_destroyed");
