                  ...
                  ULONG NumericId)
{
    ...
    for (Index = Count; Index != 0; --Index)
    {
        if (Ids[Index - 1].NumericId == NumericId)
            return Ids[Index - 1].StableId;
    }
    return NumericId;
}

static BOOL
RperfKernelSetId(RPERF_KERNEL_ID **Ids,
                 SIZE_T *Count,
                 SIZE_T *Capacity,
                 ULONG NumericId,
                 ULONGLONG StableId)
{
    ...
    {
        if ((*Ids)[Index].NumericId == NumericId)
        {
            (*Ids)[Index].StableId = StableId;
            return TRUE;
        }
    }
    if (!RperfKernelGrowIds(Ids, Capacity, *Count + 1))
        return FALSE;
    (*Ids)[*Count].NumericId = NumericId;
    (*Ids)[*Count].StableId = StableId;
    (*Count)++;
    return TRUE;
}
                      ...
                      ULONGLONG StableId)
{
    ...
    if (Thread)
    {
        return RperfKernelSetId(&State->Threads,
                                &State->ThreadCount,
                                &State->ThreadCapacity,
                                NumericId, StableId);
    }
    return RperfKernelSetId(&State->Processes,
                            &State->ProcessCount,
                        ...
                        RPERF_RECORD_HEADER *Target)
{
    ...
    Target->ProcessKey = RperfKernelFindId(State->Processes,
                                           State->ProcessCount,
                                           Source->ProcessId);
    Target->ThreadKey = RperfKernelFindId(State->Threads,
                                          State->ThreadCount,
                                          Source->ThreadId);
...
static BOOL
RperfKernelAddGap(RPERF_KERNEL_STATE *State,
                  ULONGLONG First,
                  ULONGLONG Last,
                  ULONGLONG Timestamp)
{
    ...
    Lost.Header.TimestampNs = Timestamp;
    Lost.Header.Cpu = RPERF_MODEL_ALL_CPUS;
    Lost.Data.Lost.Reason = RperfLossSequenceGap;
    Lost.Data.Lost.FirstSequence = First;
    Lost.Data.Lost.LastSequence = Last;
    Lost.Data.Lost.Count = Last - First + 1;
    return RperfRecordingAddRecord(State->Recording, &Lost);
}
                           ...
                           const ROSPROF_RECORD_HEADER *Header)
{
    ...
    switch (Header->Type)
    {
        ...
        case ROSPROF_RECORD_PROCESS:
        {
            ...
            if (Header->Size < sizeof(*Source) ||
                !RperfKernelSetId(&State->Processes,
                                  &State->ProcessCount,
                                  &State->ProcessCapacity,
                                  Header->ProcessId,
                                  Source->UniqueProcessKey))
                return FALSE;
            Record.Header.ProcessKey = Source->UniqueProcessKey;
            Record.Header.Kind = Source->Event == ROSPROF_LIFECYCLE_END ?
                                 RperfRecordProcessEnd :
                                 RperfRecordProcessStart;
            Record.Data.Lifecycle.ObjectId = Source->UniqueProcessKey;
        ...
        case ROSPROF_RECORD_THREAD:
        {
            ...
            if (Header->Size < sizeof(*Source) ||
                !RperfKernelSetId(&State->Threads,
                                  &State->ThreadCount,
                                  &State->ThreadCapacity,
                                  Header->ThreadId,
                                  Source->UniqueThreadKey))
                return FALSE;
            Record.Header.ThreadKey = Source->UniqueThreadKey;
            Record.Header.ProcessKey = Source->UniqueProcessKey;
            Record.Header.Kind = Source->Event == ROSPROF_LIFECYCLE_END ?
                                 RperfRecordThreadEnd :
                                 RperfRecordThreadStart;
            Record.Data.Lifecycle.ObjectId = Source->UniqueThreadKey;
            Record.Data.Lifecycle.ParentId = Source->UniqueProcessKey;
        ...
        case ROSPROF_RECORD_CONTEXT_SWITCH:
        {
            ...
            Record.Data.Scheduler.OldProcessKey =
                RperfKernelFindId(State->Processes, State->ProcessCount,
                                  Source->OldProcessId);
            Record.Data.Scheduler.OldThreadKey =
                RperfKernelFindId(State->Threads, State->ThreadCount,
                                  Source->OldThreadId);
    ...
    *EndOfStream = FALSE;
    ...
    for (Index = 0; Index < Batch->RecordCount; ++Index)
    {
        ...
        if (!RosProfRecordHeaderValid(Header, End - Offset) ||
            Header->Version == 0 ||
            (Index != 0 && Header->Sequence <= LastObserved))
            return FALSE;
        if (Index == 0)
            FirstObserved = Header->Sequence;
        LastObserved = Header->Sequence;
        if (State->LastSequence != 0 &&
            Header->Sequence > State->LastSequence + 1 &&
            !RperfKernelAddGap(State,
                               State->LastSequence + 1,
                               Header->Sequence - 1,
                               RperfKernelTimestampNs(State, Header->Timestamp)))
            return FALSE;
...
RperfKernelStopDevice(RPERF_KERNEL_STATE *State)
{
    ...
    Command.Reason = ROSPROF_REASON_REQUESTED;
                  ...
                  PVOID *Opaque)
{
    ...
    ROSPROF_SESSION_QUERY_V1 Query;
    DWORD Returned;
    ULONGLONG RequestedCapabilities;
    ULONG RingBytes;
    ...
    if (!RperfRosProfIoctl(State->Device,
        ...
        !RperfRosProfStructValid(&State->Capabilities.Header,
                                 ...
                                 sizeof(State->Capabilities)))
        goto Failure;
    RequestedCapabilities = ROSPROF_CAP_TIMER_SAMPLE;
    RequestedCapabilities |=
        State->Capabilities.Capabilities &
        (ROSPROF_CAP_PROCESS_LIFECYCLE |
         ROSPROF_CAP_THREAD_LIFECYCLE |
         ROSPROF_CAP_IMAGE_LIFECYCLE |
         ROSPROF_CAP_LOSS_RECORDS);
    if ((RequestedCapabilities & ~State->Capabilities.Capabilities) != 0)
    {
        SetLastError(ERROR_NOT_SUPPORTED);
        goto Failure;
    }
    ...
    if (State->Capabilities.MaximumStackDepth > 1)
    {
        ...
        Config.MaximumStackDepth = (USHORT)min(State->Capabilities.MaximumStackDepth, RPERF_MODEL_MAX_FRAMES);
