...
static ULONGLONG
RperfBaselineThreadKey(ULONG ProcessId,
                       ULONG ThreadId,
                       RPERF_BASELINE_RESULT *Result,
                       ULONGLONG *CreationTime100ns,
                       BOOL *CreationTimeValid)
{
    ...
    *CreationTimeValid = FALSE;
    ...
    OwnerProcessId = GetProcessIdOfThread(Thread);
    if (OwnerProcessId != ProcessId)
    {
        Result->Partial = TRUE;
        Result->Status = OwnerProcessId == 0 ? GetLastError() : ERROR_NOT_FOUND;
        CloseHandle(Thread);
        return RperfBaselineStableId(ThreadId, NULL,
                                     0x5448524541440000ULL);
    }
                        ...
                        RPERF_BASELINE_RESULT *Result)
{
    ...
    if (IdCallback != NULL &&
        !IdCallback(IdContext, FALSE,
                    ...
                    CreationTime100ns, CreationTimeValid))
        return FALSE;
    Result->Processes++;
    return TRUE;
}
                        ...
                        RPERF_BASELINE_RESULT *Result)
{
    ...
    if (Thread32First(Snapshot, &Entry))
    {
        do
        {
            ...
            ThreadKey = RperfBaselineThreadKey(Configuration->ProcessId,
                                               Entry.th32ThreadID,
                                               Result,
                                               &CreationTime100ns,
                                               &CreationTimeValid);
            ...
            {
                /* A numeric TID alone cannot identify a lifetime safely.
                 * The caller's partial-baseline loss record documents the
                 * omission; live backends can still use their exact key. */
                Entry.dwSize = sizeof(Entry);
                continue;
            }
            ZeroMemory(&Record, sizeof(Record));
                     ...
                     RPERF_BASELINE_RESULT *Result)
{
    ...
    ProcessKey = RperfBaselineProcessKey(Configuration->ProcessId,
                                         Result,
                                         &CreationTime100ns,
                                         &CreationTimeValid);
    if (!CreationTimeValid)
    {
        /* Process, thread-parent, and module identities all depend on this
         * lifetime.  Do not publish a numeric fallback that cannot be joined
         * to the backend's exact process key. */
        return TRUE;
    }
