                   ...
                   BOOL CreationTimeValid)
{
    ...
    UNREFERENCED_PARAMETER(CreationTime100ns);
    UNREFERENCED_PARAMETER(CreationTimeValid);
...
BOOL
RperfEtwDecodeLifecycleIds(const VOID *OpaqueData,
                           USHORT DataLength,
                           UCHAR Version,
                           ULONG PointerSize,
                           BOOL Thread,
                           ...
                           ULONG *ThreadId)
{
    ...
    ULONG ProcessOffset, ThreadOffset = 0, RequiredLength;

    if (ProcessId == NULL || ThreadId == NULL ||
        (PointerSize != 4 && PointerSize != 8) || Data == NULL)
    {
    ...
    if (Thread)
    {
        ProcessOffset = Version == 0 ? sizeof(*ThreadId) : 0;
        ThreadOffset = Version == 0 ? 0 : sizeof(*ProcessId);
        RequiredLength = 2 * sizeof(ULONG);
    }
    else
    {
        ProcessOffset = Version == 0 ? 0 : PointerSize;
        RequiredLength = ProcessOffset + sizeof(*ProcessId);
    }
    ...
    CopyMemory(ProcessId, Data + ProcessOffset, sizeof(*ProcessId));
    *ThreadId = 0;
    if (Thread)
        CopyMemory(ThreadId, Data + ThreadOffset, sizeof(*ThreadId));
    return TRUE;
}
                     ...
                     ULONG *TargetProcessId)
{
    ...
    ULONGLONG Stable;
    ULONGLONG Existing;
    ULONG Numeric, ProcessId, ThreadId;

    if (TargetProcessId != NULL)
        *TargetProcessId = 0;
    if (!RperfEtwDecodeLifecycleIds(Event->UserData,
                                    Event->UserDataLength,
    ...
    if (State->Config.Scope != RperfScopeSystem &&
        ProcessId != State->Config.ProcessId)
        return TRUE;

    RperfEtwInitializeRecord(State, Event, &Record);
    Record.Header.ProcessId = ProcessId;
    Record.Header.ProcessKey = RperfEtwFindId(State->Processes,
                                               State->ProcessCount,
                                               ProcessId);
    if (Thread)
    {
        Record.Header.ThreadId = ThreadId;
        Record.Header.ThreadKey = RperfEtwFindId(State->Threads,
                                                  State->ThreadCount,
                                                  ThreadId);
    }
    Record.Header.Kind = Ending ? EndKind : StartKind;
    Numeric = Thread ? ThreadId : ProcessId;
