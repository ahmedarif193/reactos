...
#define ONEPARAM_ROUTINE_DWMATTACH       0xfffe000f
#define ONEPARAM_ROUTINE_DWMGETFRAME     0xfffe0010
#define ONEPARAM_ROUTINE_DWMPRESENTSYNC  0xfffe0011
#define ONEPARAM_ROUTINE_DWMOPENSURFACE  0xfffe0012
...
DwmGetSurfaceView(const DWM_WIN *w)
{
    ...
    req.hSection = NULL;
    if ((LONG)NtUserCallOneParam((DWORD_PTR)&req, ONEPARAM_ROUTINE_DWMOPENSURFACE) < 0 ||
        req.hSection == NULL)
        return NULL;
...
DwmComposeLoop(void)
{
    ...
    att.Attach = 1;
    NtUserCallOneParam((DWORD_PTR)&att, ONEPARAM_ROUTINE_DWMATTACH);
    hWake = att.hWake;
    hVblank = att.hVblank;
    DwmLog("DWM: attached\n");
    ...
    for (;;)
    {
        ...
        st = (LONG)NtUserCallOneParam((DWORD_PTR)g_buf, ONEPARAM_ROUTINE_DWMGETFRAME);
        if (st < 0)
        {
            Sleep(50);
            continue;
...

        {
            ...
            if (pr > pl && pb > pt)
            {
                ...
                NtUserCallOneParam(1, ONEPARAM_ROUTINE_DWMPRESENTSYNC);
                BitBlt(hdcScreen, g_originX + pl, g_originY + pt, pr - pl, pb - pt,
                       g_hdcComp, pl, pt, SRCCOPY);
                NtUserCallOneParam(0, ONEPARAM_ROUTINE_DWMPRESENTSYNC);
            }
        }
