    ...
    _Inout_ PDWM_CONTEXT pCtx)
{
    ...
    DWORD WaitResult;
    DWORD WaitMs;
    DWORD FrameStart, FrameEnd, FrameMs;
    MSG msg;

    TRACE("DwmMessageLoop: Entering message loop (%ux%u, target %ums/frame)\n",
    ...
    /*
     * Frame pacing.  DWM_COMPOSE_INTERVAL_MS is the TARGET frame budget
     * (~16ms = 60 Hz).  Rather than always sleeping a full interval on top
     * of the compose work -- which on a CPU-bound target (no GPU, a
     * software compositor blitting a full-screen frame) is dead time that
     * caps the effective rate well below 60 Hz -- we only wait for the
     * REMAINDER of the budget that the previous frame's compose+present did
     * not already consume.  When compose already met or exceeded the budget
     * we do not add any extra delay (just a 1ms yield so other threads run),
     * so the loop runs as fast as the CPU allows up to the 60 Hz target.
     */
    WaitMs = DWM_COMPOSE_INTERVAL_MS;
    ...
    while (!pCtx->ShutdownRequested)
    {
         ...
         * with the Windows message pump.  WaitMs is the budget remainder
         * computed from the previous frame so we pace to the target without
         * adding a fixed full-interval delay on top of slow compose.
         */
        ...
        /*
         * Compute the wait for the NEXT iteration: the slice of the target
         * frame budget the compose+present did not already use.  Never less
         * than 1ms (yield the CPU); never more than the full budget.
         */
        if (FrameMs >= DWM_COMPOSE_INTERVAL_MS)
            WaitMs = 1;
        else
            WaitMs = DWM_COMPOSE_INTERVAL_MS - FrameMs;
    }
    ...
    _In_ int nCmdShow)
{
    ...
    /*
     * When launched outside the SCM (e.g. by the Explorer "Run" key on the
     * LiveCD / Pi5 image), passing "-standalone" skips the service-dispatcher
     * connect.  StartServiceCtrlDispatcherW otherwise BLOCKS ~30s waiting for
     * the SCM control pipe before failing with ERROR_FAILED_SERVICE_CONTROLLER_CONNECT
     * — a 30s delay before the compositor starts.  Skipping it starts the
     * standalone compositor immediately.
     */
    if (lpCmdLine != NULL &&
        (wcsstr(lpCmdLine, L"-standalone") != NULL ||
         wcsstr(lpCmdLine, L"/standalone") != NULL))
    {
        ForceStandalone = TRUE;
    }

    TRACE("dwm.exe: Starting service dispatcher%s\n",
          ForceStandalone ? " (skipped: -standalone)" : "");

    if (ForceStandalone || !StartServiceCtrlDispatcherW(ServiceTable))
    {
        if (!ForceStandalone)
         ...
         * command line for debugging, or with -standalone), run the
         * compositor directly.
         */
        if (ForceStandalone ||
