    ...
    _Inout_ PDWM_CONTEXT pCtx)
{
    ...
    /*
     * Phase 2 TODO: Allocate the composition buffer as a D3DKMT allocation
     * so D3DKMTPresent can flip/blit from it directly without a copy:
     *
     *   D3DKMT_CREATEALLOCATION CreateAlloc;
     *   D3DDDI_ALLOCATIONINFO   AllocInfo;
     *   ...create allocation with D3DDDIFMT_X8R8G8B8...
    ...
    _Inout_ PDWM_CONTEXT pCtx)
{
    ...
    /*
     * Phase 2 TODO: Destroy D3DKMT allocations:
    ...
    _Inout_ PDWM_CONTEXT pCtx)
{
    ...
    /*
     * For Phase 1 with display-only adapters: the dxgkrnl display.c
     * present timer reads from the shadow framebuffer that win32ss maps.
     * Our composition buffer is separate from that shadow FB.
     *
     * To get pixels on screen in Phase 1, we use GDI BitBlt to write
     * the composition buffer to the screen DC.  This is a temporary
     * bridge: Phase 2 will use D3DKMTPresent with a proper D3DKMT
     * allocation as the source.
     *
     * We still call D3DKMTPresent for correctness -- it exercises the
     * dxgkrnl present path and returns success on DOD adapters.
     */

    /* Phase 1: GDI-based screen update.
     *
     * Suppress mouse safety (cursor hide/show) during the full-screen
     * blit to avoid cursor flicker at the composition frame rate.
     * Escape code 0x44574D01 = DWM_ESCAPE_SUPPRESS_CURSOR. */
    {
        HDC hDcScreen;
        HDC hDcMem;
        HBITMAP hBitmap, hOldBitmap;
    ...
    /*
     * Phase 1 still composites into a plain heap buffer. Until this becomes
     * a real D3DKMT allocation, skip the kernel present call entirely;
     * issuing a source-less placeholder present is invalid on full WDDM.
     */
    if (pCtx->CompBuffer.hAllocation != 0)
    {
        ...
        {
            /* Non-fatal in Phase 1: GDI path already updated the screen */
            TRACE("DwmPresentFrame: D3DKMTPresent returned 0x%08lX\n", Status);
        }
        else
        {
    ...
    if (DwmpShouldLogFrame(pCtx))
    {
        TRACE("DwmPresentFrame: frame=%I64u windows=%u comp=%ux%u srcAlloc=0x%X status=0x%08lX\n",
              FrameId,
              pCtx->WindowCount,
              pCtx->CompBuffer.Width,
              pCtx->CompBuffer.Height,
