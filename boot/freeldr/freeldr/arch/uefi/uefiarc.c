    ...
    IN  ULONG  BootDeviceSize)
{
    ...
    if (UefiIsCdRomHandle(BootHandle))
    {
        ...
        {
            if (RDiskNumber)     *RDiskNumber     = UefiBootDiskArcNumber;
            if (PartitionNumber) *PartitionNumber = UefiBootDiskArcPartition;
            if (BootDevice)
                RtlStringCbPrintfA(BootDevice, BootDeviceSize,
                                   "multi(0)disk(0)rdisk(%lu)partition(%lu)",
                                   UefiBootDiskArcNumber,
                                   UefiBootDiskArcPartition);
            TRACE("UEFI ARC: Using disk alias for CD handle -> rdisk(%lu) partition(%lu)\n",
                  UefiBootDiskArcNumber,
                  UefiBootDiskArcPartition);
            return TRUE;
        }
