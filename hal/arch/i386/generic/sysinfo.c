...
#if defined(_MSC_VER)
BOOLEAN
HalpQueryAcpiRootPointerFallback(
    _Out_ PPHYSICAL_ADDRESS Address);

#pragma comment(linker, "/alternatename:HalpQueryAcpiRootPointer=HalpQueryAcpiRootPointerFallback")
#else
BOOLEAN
HalpQueryAcpiRootPointer(
    _Out_ PPHYSICAL_ADDRESS Address) __attribute__((weak));
#endif
...
#else
            if (HalpQueryAcpiRootPointer)
            {
                HasAcpi = HalpQueryAcpiRootPointer(&RsdpAddress);
            }
            else
            {
                HasAcpi = HalpQueryAcpiRootPointerFallback(&RsdpAddress);
            }
#endif
