...
BOOLEAN NTAPI HalQueryRealTimeClock(PTIME_FIELDS RtcTime) { return FALSE; }
BOOLEAN NTAPI HalSetRealTimeClock(PTIME_FIELDS RtcTime) { return FALSE; }
...
NTSTATUS NTAPI HalAllocateAdapterChannel(PADAPTER_OBJECT AdapterObject, PWAIT_CONTEXT_BLOCK Wcb, ULONG NumberOfMapRegisters, PDRIVER_CONTROL ExecutionRoutine) { return STATUS_NOT_IMPLEMENTED; }
NTSTATUS NTAPI HalAdjustResourceList(PIO_RESOURCE_REQUIREMENTS_LIST *ResourceList) { return STATUS_NOT_IMPLEMENTED; }
...
HalReportResourceUsage(VOID)
{
    ...
    /* The native image links no runtime helpers: initialize by hand. */
    HalName.Buffer = Name;
...
/* No profiling interrupt source exists on this HAL (no PMU sampling yet). */
VOID
NTAPI
