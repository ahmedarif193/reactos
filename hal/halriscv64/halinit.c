    ...
    _In_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    if (BootPhase == 1)
    {
        ...
        if (!HalpRiscvMapRtc())
        {
            HalpRiscvInitializationFailure = RiscvHalRtcMappingFailed;
            return FALSE;
        }
        if (!HalpRiscvMapPlic())
        {
    ...
    if (!HalpRiscvInitializeRtc((const VOID *)(ULONG_PTR)RiscvBlock->DeviceTree,
                                (SIZE_T)RiscvBlock->DeviceTreeSize) ||
        !HalpRiscvInitializePci((const VOID *)(ULONG_PTR)RiscvBlock->DeviceTree, (SIZE_T)RiscvBlock->DeviceTreeSize))
    {
        HalpRiscvInitializationFailure = RiscvHalInvalidDeviceTree;
        return FALSE;
    }
