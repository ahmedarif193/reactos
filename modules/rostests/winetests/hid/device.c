...
static void test_device_info(HANDLE device)
{
    ...
    ok(attributes.Size == sizeof(attributes), "Unexpected HIDD_ATTRIBUTES size: %lu\n", attributes.Size);
    ...
    if (Caps.NumberInputButtonCaps)
    {
        ...
        for (i = 1; i < 0xff; i++)
        {
            ...
            ok (status == HIDP_STATUS_SUCCESS || usage_length == 0,
                "HidP_GetUsages failed (%lx) but usage length still %lu\n", status, usage_length);
            if (usage_length)
            {
                CHAR report[50];
                int count;
...
static void test_read_device(void)
{
    ...
    if (winetest_interactive)
        trace("Test your device for the next %lu seconds\n", max_time/1000);
