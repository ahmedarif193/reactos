...
static void test_output(const char *out_data, DWORD out_size, const char *exp_data, DWORD exp_size)
{
    ...
    while(out_ptr < out_data+out_size && exp_ptr < exp_data+exp_size) {
        ...
        todo_wine_if(is_todo_wine)
        {
            ...
            if(err == out_nl)
                ok(0, "unexpected end of line %lu (got '%.*s', wanted '%.*s')\n",
                   line, (int)(out_nl-out_ptr), out_ptr, (int)(exp_nl-exp_ptr), exp_ptr);
            else if(err == exp_nl)
                ok(0, "excess characters on line %lu (got '%.*s', wanted '%.*s')\n",
                   line, (int)(out_nl-out_ptr), out_ptr, (int)(exp_nl-exp_ptr), exp_ptr);
                ...
                ok(!err, "unexpected char 0x%x position %d in line %lu (got '%.*s', wanted '%.*s')\n",
    ...
    ok(exp_ptr >= exp_data+exp_size, "unexpected end of output in line %lu, missing %s\n", line, exp_ptr);
