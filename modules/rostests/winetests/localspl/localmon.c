...
static void test_AddPort(void)
{
    ...
    */
    ...
    ok(!res, "returned %lu with %lu (expected '0')\n", res, GetLastError());
    ...
    for(level = 0; level < 4; level++) {
        ...
        if(!level || (level > 2)) {
            ...
            ok( (cbBuf == 0) && (pcReturned == 0),
                "(%lu) returned %lu with %lu and %lu, %lu (expected 0, 0)\n",
                level, res, GetLastError(), cbBuf, pcReturned);
            continue;
        }        

        ok( !res && (GetLastError() == ERROR_INSUFFICIENT_BUFFER),
            "(%lu) returned %lu with %lu and %lu, %lu (expected '0' with "
        ...
        ok( res, "(%lu) returned %lu with %lu and %lu, %lu (expected '!= 0')\n",
...
static void test_XcvClosePort(void)
{
    ...
    ok(res, "returned %lu with %lu and %p (expected '!= 0')\n", res, GetLastError(), hXcv2);
...
static void test_XcvDataPort_AddPort(void)
{
    ...
    ok( res == ERROR_SUCCESS, "returned %lu with %lu (expected ERROR_SUCCESS)\n", res, GetLastError());

    /* add our testport again */
    SetLastError(0xdeadbeef);
    res = pXcvDataPort(hXcv, cmd_AddPortW, (PBYTE) tempfileW, (lstrlenW(tempfileW) + 1) * sizeof(WCHAR), NULL, 0, NULL);
    ok( res == ERROR_ALREADY_EXISTS, "returned %lu with %lu (expected ERROR_ALREADY_EXISTS)\n", res, GetLastError());
...
static void test_XcvDataPort_ConfigureLPTPortCommandOK(void)
{
    ...
    ok( (res == ERROR_SUCCESS) && (lstrcmpA(buffer, num_0A) == 0),
        "returned %lu and '%s' (expected ERROR_SUCCESS and '%s')\n",
        res, buffer, num_0A);
...
static void test_XcvDataPort_DeletePort(void)
{
    ...
    ok( !res  || (res == ERROR_FILE_NOT_FOUND),
        "returned %lu with %lu (expected ERROR_SUCCESS or ERROR_FILE_NOT_FOUND)\n",
        res, GetLastError());
    ...
    ok( res == ERROR_FILE_NOT_FOUND,
        "returned %lu with %lu (expected ERROR_FILE_NOT_FOUND)\n", res, GetLastError());
...
static void test_XcvDataPort_GetTransmissionRetryTimeout(void)
{
    ...
    ok( (res == ERROR_INSUFFICIENT_BUFFER) && (needed == len),
        "returned %lu with %lu and %lu (expected ERROR_INSUFFICIENT_BUFFER "
    ...
    ok( (res == ERROR_SUCCESS) && (buffer[0] == 45),
        "returned %lu with %lu and %lu for %lu\n (expected ERROR_SUCCESS "
...
static void test_XcvDataPort_MonitorUI(void)
{
    ...
    ok( (res == ERROR_INSUFFICIENT_BUFFER) && (needed <= MAX_PATH),
        "returned %lu with %lu and 0x%lx (expected 'ERROR_INSUFFICIENT_BUFFER' "
    ...
    ok( res == ERROR_INVALID_PARAMETER, "returned %lu with %lu and 0x%lx "
    ...
    ok( res == ERROR_SUCCESS, "returned %lu with %lu and 0x%lx "
        "(expected 'ERROR_SUCCESS')\n", res, GetLastError(), needed);
    ...
    SetLastError(0xdeadbeef);
    res = pXcvDataPort(hXcv, cmd_MonitorUIW, NULL, 0, buffer, len-1, &needed);
    ok( res == ERROR_INSUFFICIENT_BUFFER, "returned %lu with %lu and 0x%lx "
...
static void test_XcvDataPort_PortIsValid(void)
{
    ...
    ok( res == ERROR_SUCCESS,
        "returned %lu with %lu and 0x%lx (expected ERROR_SUCCESS)\n",
        res, GetLastError(), needed);
    ...
    ok( res == ERROR_PATH_NOT_FOUND,
        "returned %lu with %lu and 0x%lx (expected ERROR_PATH_NOT_FOUND)\n",
        res, GetLastError(), needed);
    ...
    ok( (res == ERROR_INVALID_NAME) || (res == ERROR_PATH_NOT_FOUND) ||
        (res == ERROR_ACCESS_DENIED), "returned %lu with %lu and 0x%lx "
...
START_TEST(localmon)
{
    ...
    if (pm || pm2) {
        ...
        if ((pXcvOpenPort) && (pXcvDataPort) && (pXcvClosePort)) {
            ...
            ok(res, "hXcv: %lu with %lu and %p (expected '!= 0')\n", res, GetLastError(), hXcv);

            SetLastError(0xdeadbeef);
            res = pXcvOpenPort(emptyW, 0, &hXcv_noaccess);
            ok(res, "hXcv_noaccess: %lu with %lu and %p (expected '!= 0')\n", res, GetLastError(), hXcv_noaccess);
        }
    }
