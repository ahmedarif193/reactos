...
static BOOL
CallNullD3DKMT(FARPROC Proc, NTSTATUS *Status, DWORD *ExceptionCode)
{
    ...
    *ExceptionCode = 0;
    ...
    _SEH2_TRY
    {
        *Status = ((PFN_D3DKMT_NULLPROC)Proc)(NULL);
    }
...
TestNullD3DKMTCall(const char *Name, FARPROC Proc)
{
    ...
    if (!CallNullD3DKMT(Proc, &Status, &ExceptionCode))
    {
        ok(ExceptionCode == EXCEPTION_ACCESS_VIOLATION,
           "%s(NULL) raised 0x%lx\n", Name, ExceptionCode);
        return;
    }
    ...
    ok(Status == STATUS_INVALID_PARAMETER,
       "%s(NULL) returned 0x%lx\n", Name, Status);
}

static void Test_CreateAllocation_NullParam(void)
{
    LOAD_D3DKMT(D3DKMTCreateAllocation);
    TestNullD3DKMTCall("D3DKMTCreateAllocation", (FARPROC)pfnD3DKMTCreateAllocation);
}
...
static void Test_CreateDestroyAllocation(void)
{
    ...
    CreateData.pAllocationInfo = NULL;
    ...
    Status = pfnD3DKMTCreateAllocation(&CreateData);
    ok(Status == STATUS_INVALID_PARAMETER,
       "D3DKMTCreateAllocation with NULL allocation info returned 0x%lx\n",
       Status);
