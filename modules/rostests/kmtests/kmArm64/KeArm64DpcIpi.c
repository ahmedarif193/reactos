...
static VOID Arm64DpcIpiCheck(VOID)
{
    ...
    /* RequestMailbox is the 64-byte REQUEST_MAILBOX struct followed by
     * 64 bytes of padding (total 128 = 0x80). */
    ok_eq_size(sizeof(Prcb->RequestMailbox) + sizeof(Prcb->RequestMailboxPad),
               (SIZE_T)0x80);
    ok_eq_size(sizeof(Prcb->RequestMailbox), (SIZE_T)0x40);

    /* Mailbox / TargetCount / IpiFrozen reachable. */
    {
        UINT64 Mb = Prcb->Mailbox;
