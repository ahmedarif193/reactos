...
def wrapper(name, declaration, target=None):
    ...
    if "..." in signature or opening < 0:
        return None
    result_type = normalized_type(signature[:opening]).replace("__size_t", "size_t")
    if not result_type or "(" in result_type:
        return None
    params = [normalized_type(param) for param in declaration["params"]]
...
def normalized_type(typ):
    """Discard Clang nullability sugar that is not part of the bridge ABI."""
    return re.sub(r"\s+_(?:Nonnull|Nullable|Null_unspecified)\b", "", typ).strip()
