...
static BOOL AVISplitter_IsStreamConnected(AVISplitterImpl *This, DWORD streamnumber)
{
    IPin *peer;

    if (IPin_ConnectedTo(This->Parser.ppPins[streamnumber + 1], &peer) != S_OK)
        return FALSE;
...
static HRESULT AVISplitter_first_request(LPVOID iface)
{
    ...
    for (x = 0; x < This->Parser.cStreams; ++x)
    {
        ...
        stream->first_request_pending = FALSE;
        /* Nothing should be running at this point */
        assert(!stream->thread);

        if (!AVISplitter_IsStreamConnected(This, x))
            continue;
        ...
        have_sample = (hr == S_OK);
        stream->first_request_pending = have_sample;
        if (hr == S_FALSE)
    ...
    for (x = 0; x < This->Parser.cStreams && SUCCEEDED(hr); ++x)
    {
        ...
        if (!This->streams[x].first_request_pending)
            continue;
