...
{
    static const struct { GUID id; int csidl; } folder_map[] =
    {
        { {0x3EB685DB,0x65F9,0x4CF6,{0xA0,0x3A,0xE3,0xEF,0x65,0x72,0x9F,0x3D}}, CSIDL_APPDATA },
        { {0xF1B32785,0x6FBA,0x4FCF,{0x9D,0x55,0x7B,0x8E,0x7F,0x15,0x70,0x91}}, CSIDL_LOCAL_APPDATA },
        { {0x62AB5D82,0xFDC1,0x4DC3,{0xA9,0xDD,0x07,0x0D,0x1D,0x49,0x5D,0x97}}, CSIDL_COMMON_APPDATA },
        { {0x5E6C858F,0x0E22,0x4760,{0x9A,0xFE,0xEA,0x33,0x17,0xB6,0x71,0x73}}, CSIDL_PROFILE },
        { {0x905E63B6,0xC1BF,0x494E,{0xB2,0x9C,0x65,0xB7,0x32,0xD3,0xD2,0x1A}}, CSIDL_PROGRAM_FILES },
        { {0x7C5A40EF,0xA0FB,0x4BFC,{0x87,0x4A,0xC0,0xF2,0xE0,0xB9,0xFA,0x8E}}, CSIDL_PROGRAM_FILESX86 },
        { {0xF38BF404,0x1D43,0x42F2,{0x93,0x05,0x67,0xDE,0x0B,0x28,0xFC,0x23}}, CSIDL_WINDOWS },
        { {0x1AC14E77,0x02E7,0x4E5D,{0xB7,0x44,0x2E,0xB1,0xAE,0x51,0x98,0xB7}}, CSIDL_SYSTEM },
        { {0xFDD39AD0,0x238F,0x46AF,{0xAD,0xB4,0x6C,0x85,0x48,0x03,0x69,0xC7}}, CSIDL_PERSONAL },
        { {0xB4BFCC3A,0xDB2C,0x424C,{0xB0,0x29,0x7F,0xE9,0x9A,0x87,0xC6,0x41}}, CSIDL_DESKTOPDIRECTORY },
    };
    WCHAR szPath[MAX_PATH];
    int csidl = -1;
    SIZE_T i, cb;
    ...
    for (i = 0; i < ARRAY_SIZE(folder_map); i++)
    {
        if (IsEqualGUID(rfid, &folder_map[i].id))
        {
            csidl = folder_map[i].csidl;
            break;
        }
    }
    if (csidl < 0)
