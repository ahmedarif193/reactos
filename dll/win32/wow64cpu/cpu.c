...
NTSTATUS WINAPI BTCpuProcessInit(void)
{
    ...
    RtlWow64GetThreadContext( NtCurrentThread(), &context_i386 );
    ...
    NtProtectVirtualMemory( NtCurrentProcess(), (void **)&thunk, &size, PAGE_EXECUTE_READ, &old_prot );
...
NTSTATUS WINAPI BTCpuResetToConsistentState( EXCEPTION_POINTERS *ptrs )
{
    ...
    copy_context_64to32( &wow_context, CONTEXT_I386_ALL, (AMD64_CONTEXT *)context );
    wow_context.EFlags &= ~(0x100|0x40000);
    BTCpuSetContext( NtCurrentThread(), NtCurrentProcess(), NULL, &wow_context );
