...
static void init_filedlg_infoW(OPENFILENAMEW *ofn, FileOpenDlgInfos *info)
{
    ...
    if (ofn->lpstrDefExt)
    {
        ...
        LPWSTR pszExt = malloc((cchExt + 1) * sizeof(WCHAR));
        ...
        case CDM_SETDEFEXT:
        {
            ...
            if (fodInfos->unicode)
            {
                ...
                if (pszExt)
                {
                    ...
                    fodInfos->defext = malloc((cchExt + 1) * sizeof(WCHAR));
...
FILEDLG95_AddDotExtIfNeeded(FileOpenDlgInfos *fodInfos, LPWSTR lpstrPathAndFile)
{
    ...
    {
        if (wcslen(lpstrPathAndFile) + 1 + wcslen(the_ext) + 1 <=
    ...
    if (*ext)
        ext++;
    if (!fodInfos->defext || !lstrcmpiW(fodInfos->defext, ext))
...
#ifdef __REACTOS__
            if((dwAttributes & (SFGAO_FOLDER | SFGAO_FILESYSANCESTOR)) ==
