...
NTSTATUS WINAPI wow64_NtGdiAlphaBlend( UINT *args )
{
    ...
    return NtGdiAlphaBlend( hdc_dst, x_dst, y_dst, width_dst, height_dst, hdc_src, x_src, y_src, width_src, height_src, blend_function, xform );
}
...
NTSTATUS WINAPI wow64_NtGdiBitBlt( UINT *args )
{
    ...
    return NtGdiBitBlt( hdc_dst, x_dst, y_dst, width, height, hdc_src, x_src, y_src, rop, bk_color, fl );
}
...
NTSTATUS WINAPI wow64_NtGdiCreateDIBSection( UINT *args )
{
    ...
    ret = NtGdiCreateDIBSection( hdc, section, offset, bmi, usage, header_size, flags, color_space, addr_32to64( &bits, bits32 ));
...
NTSTATUS WINAPI wow64_NtGdiCreateDIBitmapInternal( UINT *args )
{
    ...
    HBITMAP ret = NtGdiCreateDIBitmapInternal( hdc, width, height, init, bits, data, coloruse, max_info, max_bits, flags, xform );
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtGdiCreateRoundRectRgn( UINT *args )
{
    ...
    return HandleToUlong( NtGdiCreateRoundRectRgn( left, top, right, bottom, ellipse_width, ellipse_height ));
}
...
NTSTATUS WINAPI wow64_NtGdiExtCreatePen( UINT *args )
{
    ...
    return HandleToUlong( NtGdiExtCreatePen( style, width, brush_style, color, client_hatch, hatch, style_count, style_bits, dib_size, old_style, brush ));
}
...
NTSTATUS WINAPI wow64_NtGdiGetDIBitsInternal( UINT *args )
{
    ...
    return NtGdiGetDIBitsInternal( hdc, hbitmap, startscan, lines, bits, info, coloruse, max_bits, max_info );
}
...
#ifdef __REACTOS__
NTSTATUS WINAPI wow64_NtGdiGetTextExtent( UINT *args )
{
    HDC hdc = get_handle( &args );
    const WCHAR *str = get_ptr( &args );
    INT count = get_ulong( &args );
...
NTSTATUS WINAPI wow64_NtGdiMaskBlt( UINT *args )
{
    ...
    return NtGdiMaskBlt( hdc, x_dst, y_dst, width_dst, height_dst, hdc_src, x_src, y_src, mask, x_mask, y_mask, rop, bk_color );
}
...
NTSTATUS WINAPI wow64_NtGdiPlgBlt( UINT *args )
{
    ...
    return NtGdiPlgBlt( hdc, point, hdc_src, x_src, y_src, width, height, mask, x_mask, y_mask, bk_color );
}
...
NTSTATUS WINAPI wow64_NtGdiSetDIBitsToDeviceInternal( UINT *args )
{
    ...
    return NtGdiSetDIBitsToDeviceInternal( hdc, x_dst, y_dst, cx, cy, x_src, y_src, startscan, lines, bits, bmi, coloruse, max_bits, max_info, xform_coords, xform );
}
...
NTSTATUS WINAPI wow64_NtGdiStretchBlt( UINT *args )
{
    ...
    return NtGdiStretchBlt( hdc, x_dst, y_dst, width_dst, height_dst, hdc_src, x_src, y_src, width_src, height_src, rop, bk_color );
}
...
NTSTATUS WINAPI wow64_NtGdiStretchDIBitsInternal( UINT *args )
{
    ...
    return NtGdiStretchDIBitsInternal( hdc, x_dst, y_dst, width_dst, height_dst, x_src, y_src, width_src, height_src, bits, bmi, coloruse, rop, max_info, max_bits, xform );
}
...
NTSTATUS WINAPI wow64_NtGdiTransparentBlt( UINT *args )
{
    ...
    return NtGdiTransparentBlt( hdc, x_dst, y_dst, width_dst, height_dst, hdc_src, x_src, y_src, width_src, height_src, color );
}
