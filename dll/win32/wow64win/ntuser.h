...
W32KAPI ULONG   WINAPI NtUserGetWindowDpiAwarenessContext( HWND hwnd );
...
W32KAPI ULONG   WINAPI NtUserSetThreadDpiAwarenessContext( ULONG context );
...
{
    return NtUserCallTwoParam( HandleToUlong(menu), (ULONG_PTR)info, NtUserCallTwoParam_GetMenuInfo );
}

#ifdef __REACTOS__
W32KAPI BOOL WINAPI NtUserGetMonitorInfo( HMONITOR monitor, MONITORINFO *info );
#else
static inline BOOL NtUserGetMonitorInfo( HMONITOR monitor, MONITORINFO *info )
{
    return NtUserCallTwoParam( HandleToUlong(monitor), (ULONG_PTR)info, NtUserCallTwoParam_GetMonitorInfo );
}
#endif
...
#ifdef __REACTOS__
W32KAPI HMONITOR WINAPI NtUserMonitorFromPoint( POINT point, DWORD flags );
W32KAPI HMONITOR WINAPI NtUserMonitorFromRect( const RECT *rect, DWORD flags );
#else
...
{
    return UlongToHandle( NtUserCallHwndParam( hwnd, rel, NtUserCallHwndParam_GetWindowRelative ));
}
...
static inline int NtUserMapWindowPoints( HWND hwnd_from, HWND hwnd_to, POINT *points, UINT count, UINT dpi )
{
    ...
    params.count = count;
    params.dpi = dpi;
    return NtUserCallHwndParam( hwnd_from, (UINT_PTR)&params, NtUserCallHwndParam_MapWindowPoints );
}
...
#ifdef __REACTOS__
W32KAPI HMONITOR WINAPI NtUserMonitorFromWindow( HWND hwnd, DWORD flags );
#else
