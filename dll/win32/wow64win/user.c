...
static inline HANDLE get_user_handle( UINT **args ) { return Handle32ToHandle( *(*args)++ ); }
static inline HANDLE get_kernel_handle( UINT **args ) { return LongToHandle( *(*args)++ ); }
#define get_handle get_user_handle

static const ROS_SERVERINFO64_PFN *ros_server_info;
static ROS_PFNCLIENT32 ros_client_procs_a;
static ROS_PFNCLIENT32 ros_client_procs_w;
static ROS_PFNCLIENTWORKER32 ros_client_workers;
static BOOL ros_client_procs_initialized;
#endif
...
#ifdef __REACTOS__
static void ros_cursordata_32to64( ROS_CURSORDATA64 *data, const ROS_CURSORDATA32 *data32 )
{
    data->lpName = data32->lpName;
    data->lpModName = data32->lpModName;
    data->rt = data32->rt;
    data->dummy = data32->dummy;
    data->flags = data32->flags;
    data->xHotspot = data32->xHotspot;
    data->yHotspot = data32->yHotspot;
    data->hbmMask = data32->hbmMask;
    data->hbmColor = data32->hbmColor;
    data->hbmAlpha = data32->hbmAlpha;
    data->rcBounds = data32->rcBounds;
    data->hbmUserAlpha = data32->hbmUserAlpha;
    data->bpp = data32->bpp;
    data->cx = data32->cx;
    data->cy = data32->cy;
    data->cpcur = data32->cpcur;
    data->cicur = data32->cicur;
    data->aspcur = data32->aspcur;
    data->aicur = data32->aicur;
    data->ajifRate = data32->ajifRate;
    data->iicur = data32->iicur;
}
#endif
                                                    ...
                                                    MOUSEHOOKSTRUCTEX32 *hook32 )
{
    ...
    hook.pt           = hook64->MOUSEHOOKSTRUCT.pt;
    hook.hwnd         = HandleToUlong( hook64->MOUSEHOOKSTRUCT.hwnd );
    hook.wHitTestCode = hook64->MOUSEHOOKSTRUCT.wHitTestCode;
    hook.dwExtraInfo  = hook64->MOUSEHOOKSTRUCT.dwExtraInfo;
...
static NTSTATUS WINAPI wow64_NtUserCallWinEventHook( void *arg, ULONG size )
{
    ...
    return dispatch_callback( NtUserCallWinEventHook, &params32, FIELD_OFFSET( struct win_event_hook_params32, module ) + size);
}
                                     ...
                                     const void *params64, void *params32, size_t size )
{
    ...
    case WM_NCCALCSIZE:
        if (wparam)
        {
            ...
            winpos_64to32( (const WINDOWPOS *)(ncp64 + 1), (WINDOWPOS32 *)((const char *)params32 + sizeof(ncp32)) );
...
static NTSTATUS WINAPI wow64_NtUserCallWinProc( void *arg, ULONG size )
{
    ...
    if (size > sizeof(*params))
    {
        ...
        lparam_size = packed_message_64to32( params32->msg, params32->wparam, (char *)params + offset64, (char *)params32 + offset32, size - offset64 );
    }

    status = Wow64KiUserCallbackDispatcher( NtUserCallWinProc, params32, offset32 + lparam_size, &ret_ptr, &ret_len );

    if (ret_len >= sizeof(LONG))
    {
        LRESULT *result_ptr = arg;
        result = *(LONG *)ret_ptr;
        ret_len = packed_result_32to64( params32->msg, params32->wparam, (LONG *)ret_ptr + 1, ret_len - sizeof(LONG), result_ptr + 1 );
        *result_ptr = result;
    ...
    case WH_CALLWNDPROC:
        {
            ...
            if (size > sizeof(*cwp))
            {
                ...
                size = packed_message_64to32( cwp32.message, cwp32.wParam, (const char *)lp + offset64, (char *)lp32 + offset32, size - offset64 );
    ...
    case WH_CALLWNDPROCRET:
        {
            ...
            if (size > sizeof(*cwpret))
            {
                ...
                size = packed_message_64to32( cwpret32.message, cwpret32.wParam, (const char *)lp + offset64, (char *)lp32 + offset32, size - offset64 );
                return offset32 + size;
            }
...
static NTSTATUS WINAPI wow64_NtUserCallWindowsHook( void *arg, ULONG size )
{
    ...
    memmove( ((struct win_hook_params32 *)arg)->module, params->module, (module_len + 1) * sizeof(WCHAR) );

    if (size > offset)
    {
        size32 = (size32 + 15) & ~15;
        offset = (offset + 15) & ~15;
        size32 += hook_lparam_64to32( params32.id, params32.code, (char *)params + offset, size - offset, (char *)arg + size32 );
    }
...
static NTSTATUS WINAPI wow64_NtUserCopyImage( void *arg, ULONG size )
{
    ...
    status = Wow64KiUserCallbackDispatcher( NtUserCopyImage, &params32, sizeof(params32), &ret_ptr, &ret_len );
...
static NTSTATUS WINAPI wow64_NtUserLoadImage( void *arg, ULONG size )
{
    ...
    params32.dy = params->dy;
    params32.flags = params->flags;
    status = Wow64KiUserCallbackDispatcher( NtUserLoadImage, &params32, sizeof(params32), &ret_ptr, &ret_len );
    if (!status && ret_len == sizeof(ULONG))
    {
        HANDLE handle = ULongToHandle( *(ULONG *)ret_ptr );
        return NtCallbackReturn( &handle, sizeof(handle), status );
...
NTSTATUS WINAPI wow64_NtUserBuildHwndList( UINT *args )
{
    ...
    if ((status = NtUserBuildHwndList( desktop, hwnd, children, non_immersive, thread_id, count, buffer, size )))
        return status;
...
NTSTATUS WINAPI wow64_NtUserCreateDesktopEx( UINT *args )
{
    ...
    ret = NtUserCreateDesktopEx( objattr_32to64( &attr, attr32 ), unicode_str_32to64( &device, device32 ), devmode, flags, access, heap_size );
    return HandleToUlong( ret );
}
...
#ifdef __REACTOS__
    ROS_LARGE_STRING32 *class_name32 = get_ptr( &args );
    ROS_LARGE_STRING32 *version32 = get_ptr( &args );
    ROS_LARGE_STRING32 *window_name32 = get_ptr( &args );
#else
...
#ifdef __REACTOS__
    ROS_LARGE_STRING64 class_name, version, window_name;
#else
...
#ifdef __REACTOS__
    ret = ((ROS_NTUSER_CREATE_WINDOW_EX)NtUserCreateWindowEx)(ex_style, ros_large_str_32to64(&class_name, class_name32), ros_large_str_32to64(&version, version32), ros_large_str_32to64(&window_name, window_name32), style, x, y, width, height, parent, menu, instance, params, flags, acbi_buffer);
#else
    ret = NtUserCreateWindowEx( ex_style, unicode_str_32to64( &class_name, class_name32), unicode_str_32to64( &version, version32 ), unicode_str_32to64( &window_name, window_name32 ), style, x, y, width, height, parent, menu, instance, params, flags, client_instance, class, ansi );
#endif
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtUserCreateWindowStation( UINT *args )
{
    ...
    return HandleToUlong( NtUserCreateWindowStation( objattr_32to64( &attr, attr32 ), access, arg3, arg4, arg5, arg6, arg7 ));
}
...
NTSTATUS WINAPI wow64_NtUserDefSetText( UINT *args )
{
    HWND hwnd = get_handle( &args );
    ROS_LARGE_STRING32 *text32 = get_ptr( &args );
    ROS_LARGE_STRING64 text;

    return NtUserDefSetText(hwnd, ros_large_str_32to64(&text, text32));
}
...
NTSTATUS WINAPI wow64_NtUserEnumDisplaySettings( UINT *args )
{
    ...
    return NtUserEnumDisplaySettings( unicode_str_32to64( &device, device32 ), mode, dev_mode, flags );
}
...
NTSTATUS WINAPI wow64_NtUserFindExistingCursorIcon( UINT *args )
{
    ...
    ret = NtUserFindExistingCursorIcon( unicode_str_32to64( &module, module32 ), unicode_str_32to64( &res_name, res_name32 ), desc );
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtUserFindWindowEx( UINT *args )
{
    ...
    ret = NtUserFindWindowEx( parent, child, unicode_str_32to64( &class, class32 ), unicode_str_32to64( &title, title32 ), unk );
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtUserGetClassInfoEx( UINT *args )
{
    ...
    wc.cbSize = sizeof(wc);
    if (!(ret = NtUserGetClassInfoEx( instance, unicode_str_32to64( &name, name32 ), &wc, &menu_name, ansi )))
        return 0;
...
#ifdef __REACTOS__
NTSTATUS WINAPI wow64_NtUserGetCPD( UINT *args )
{
    HWND hwnd = get_handle( &args );
    UINT flags = get_ulong( &args );
    ULONG_PTR proc = get_ulong( &args );

    return NtUserGetCPD(hwnd, flags, proc);
}

NTSTATUS WINAPI wow64_NtUserGetControlBrush( UINT *args )
{
    HWND hwnd = get_handle( &args );
    HDC hdc = get_handle( &args );
    UINT message = get_ulong( &args );

    return HandleToUlong(NtUserGetControlBrush(hwnd, hdc, message));
}

NTSTATUS WINAPI wow64_NtUserGetControlColor( UINT *args )
{
    HWND parent = get_handle( &args );
    HWND hwnd = get_handle( &args );
    HDC hdc = get_handle( &args );
    ...
    return HandleToUlong(NtUserGetControlColor(parent, hwnd, hdc, message));
}
#endif
...
#ifdef __REACTOS__
    if (!NtUserGetIconInfo( icon, info32 ? &info : NULL, unicode_str_32to64( &module, module32 ), unicode_str_32to64( &res_name, res_name32 ), bpp, unk ))
#else
    if (!NtUserGetIconInfo( icon, &info, unicode_str_32to64( &module, module32 ), unicode_str_32to64( &res_name, res_name32 ), bpp, unk ))
#endif
        return FALSE;
...
#ifdef __REACTOS__
NTSTATUS WINAPI wow64_NtUserGetMonitorInfo( UINT *args )
{
    HMONITOR monitor = get_handle( &args );
    MONITORINFO *info = get_ptr( &args );
...
{
    HANDLE process = get_kernel_handle( &args );
...
NTSTATUS WINAPI wow64_NtUserRegisterClassExWOW( UINT *args )
{
    ...
    UNICODE_STRING32 *version32 = get_ptr( &args );
#ifdef __REACTOS__
    const ROS_CLSMENUNAME32 *menu_name32 = get_ptr( &args );
#else
...
#ifdef __REACTOS__
    UNICODE_STRING name, version, menu_string;
    ROS_CLSMENUNAME64 menu_name;
#else
...
#ifdef __REACTOS__
    menu_name.pszClientAnsiMenuName = menu_name32->pszClientAnsiMenuName;
    menu_name.pwszClientUnicodeMenuName = menu_name32->pwszClientUnicodeMenuName;
    menu_name.pusMenuName = (ULONG_PTR)unicode_str_32to64( &menu_string, ULongToPtr( menu_name32->pusMenuName ));
    return NtUserRegisterClassExWOW( &wc, unicode_str_32to64( &name, name32 ), unicode_str_32to64( &version, version32 ), (struct client_menu_name *)&menu_name, fnid, flags, wow );
#else
    return NtUserRegisterClassExWOW( &wc, unicode_str_32to64( &name, name32 ), unicode_str_32to64( &version, version32 ), menu_name, fnid, flags, wow );
#endif
}
                                    ...
                                    void *result_info, DWORD type, BOOL ansi )
{
    ...
    case WM_GETDLGCODE:
        if (lparam)
        {
            ...
            return NtUserMessageCall( hwnd, msg, wparam, (LPARAM)msg_32to64( &msg64, msg32 ), result_info, type, ansi );
        }
        return NtUserMessageCall( hwnd, msg, wparam, lparam, result_info, type, ansi );
    ...
    case NtUserDefWindowProc:
        {
            ...
            status = message_call_32to64( hwnd, msg, wparam, lparam, &result, type, ansi );
            if (result_info) *(LONG *)result_info = result;
            return status;
        }
#endif
    ...
    case NtUserSendMessage:
        {
            ...
            status = message_call_32to64(hwnd, msg, wparam, lparam, &result, type, ansi);
            if (result_info) *(LONG *)result_info = (LONG)result;
            return status;
        }
#else
    case NtUserSendMessage:
...
#ifdef __REACTOS__
NTSTATUS WINAPI wow64_NtUserMonitorFromPoint( UINT *args )
{
    POINT point;
    point.x = get_ulong( &args );
    point.y = get_ulong( &args );
    DWORD flags = get_ulong( &args );

    return HandleToUlong(NtUserMonitorFromPoint(point, flags));
}

NTSTATUS WINAPI wow64_NtUserMonitorFromRect( UINT *args )
{
    const RECT *rect = get_ptr( &args );
    DWORD flags = get_ulong( &args );

    return HandleToUlong(NtUserMonitorFromRect(rect, flags));
}

NTSTATUS WINAPI wow64_NtUserMonitorFromWindow( UINT *args )
{
    HWND hwnd = get_handle( &args );
    DWORD flags = get_ulong( &args );

    return HandleToUlong(NtUserMonitorFromWindow(hwnd, flags));
}
#endif
...
#ifdef __REACTOS__
NTSTATUS WINAPI wow64_NtUserProcessConnect( UINT *args )
{
    HANDLE process = get_kernel_handle( &args );
    ROS_USERCONNECT32 *connect32 = get_ptr( &args );
    ULONG size = get_ulong( &args );
    ROS_USERCONNECT64 connect;
    NTSTATUS status;
    UINT i;

    if (!connect32 || size != sizeof(*connect32)) return STATUS_UNSUCCESSFUL;

    memset( &connect, 0, sizeof(connect) );
    connect.ulVersion = connect32->ulVersion;
    connect.ulCurrentVersion = connect32->ulCurrentVersion;
    connect.dwDispatchCount = connect32->dwDispatchCount;
    status = NtUserProcessConnect( process, &connect, sizeof(connect) );
    if (!NT_SUCCESS(status)) return status;

    if (connect.siClient.psi > MAXDWORD || connect.siClient.aheList > MAXDWORD || connect.siClient.pDispInfo > MAXDWORD) return STATUS_INVALID_ADDRESS;
    ros_server_info = (const ROS_SERVERINFO64_PFN *)(ULONG_PTR)connect.siClient.psi;
    if (connect.siClient.DefWindowMsgs.abMsgs > MAXDWORD || connect.siClient.DefWindowSpecMsgs.abMsgs > MAXDWORD) return STATUS_INVALID_ADDRESS;
    for (i = 0; i < ARRAY_SIZE(connect.siClient.awmControl); ++i)
        if (connect.siClient.awmControl[i].abMsgs > MAXDWORD) return STATUS_INVALID_ADDRESS;

    connect32->ulVersion = connect.ulVersion;
    connect32->ulCurrentVersion = connect.ulCurrentVersion;
    connect32->dwDispatchCount = connect.dwDispatchCount;
    connect32->siClient.psi = connect.siClient.psi;
    connect32->siClient.aheList = connect.siClient.aheList;
    connect32->siClient.pDispInfo = connect.siClient.pDispInfo;
    connect32->siClient.ulSharedDelta = connect.siClient.ulSharedDelta;
    for (i = 0; i < ARRAY_SIZE(connect.siClient.awmControl); ++i)
    {
        connect32->siClient.awmControl[i].maxMsgs = connect.siClient.awmControl[i].maxMsgs;
        connect32->siClient.awmControl[i].abMsgs = connect.siClient.awmControl[i].abMsgs;
    }
    connect32->siClient.DefWindowMsgs.maxMsgs = connect.siClient.DefWindowMsgs.maxMsgs;
    connect32->siClient.DefWindowMsgs.abMsgs = connect.siClient.DefWindowMsgs.abMsgs;
    connect32->siClient.DefWindowSpecMsgs.maxMsgs = connect.siClient.DefWindowSpecMsgs.maxMsgs;
    connect32->siClient.DefWindowSpecMsgs.abMsgs = connect.siClient.DefWindowSpecMsgs.abMsgs;
    return status;
}
#endif
...
NTSTATUS WINAPI wow64_NtUserSetCursorIconData( UINT *args )
{
    ...
    UNICODE_STRING32 *res_name32 = get_ptr( &args );
#ifdef __REACTOS__
    const ROS_CURSORDATA32 *data32 = get_ptr( &args );
    ROS_CURSORDATA64 data, *frames;
    UNICODE_STRING module, res_name;
    UINT i;

    ros_cursordata_32to64( &data, data32 );
    if (data.flags & 0x0008)
    {
        if (!data.cpcur || data.cpcur > 1000) return FALSE;
        if (!(frames = Wow64AllocateTemp( data.cpcur * sizeof(*frames) ))) return FALSE;
        for (i = 0; i < data.cpcur; i++) ros_cursordata_32to64( &frames[i], &((const ROS_CURSORDATA32 *)ULongToPtr( data32->aspcur ))[i] );
        data.aspcur = (ULONG_PTR)frames;
    }

    return NtUserSetCursorIconData( cursor, unicode_str_32to64( &module, module32 ), unicode_str_32to64( &res_name, res_name32 ), (struct cursoricon_desc *)&data );
#else
    struct
    {
        UINT  flags;
    ...
    return NtUserSetCursorIconData( cursor, unicode_str_32to64( &module, module32 ), unicode_str_32to64( &res_name, res_name32), &desc );
#endif
}
...
NTSTATUS WINAPI wow64_NtUserSetWinEventHook( UINT *args )
{
    ...
    ret = NtUserSetWinEventHook( event_min, event_max, inst, unicode_str_32to64( &module, module32 ), proc, pid, tid, flags );
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtUserSetWindowsHookEx( UINT *args )
{
    ...
    ret = NtUserSetWindowsHookEx( inst, unicode_str_32to64( &module, module32 ), tid, id, proc, ansi );
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtUserThunkedMenuItemInfo( UINT *args )
{
    ...
    ret = NtUserThunkedMenuItemInfo( handle, pos, flags, method, info_ptr, unicode_str_32to64( &str, str32 ));
...
NTSTATUS WINAPI wow64_NtUserUpdateLayeredWindow( UINT *args )
{
    ...
    return NtUserUpdateLayeredWindow( hwnd, hdc_dst, pts_dst, size, hdc_src, pts_src, key, blend, flags, dirty );
}
