...
#ifdef __REACTOS__
WCHAR wow64_windows_dir[MAX_PATH];
WCHAR wow64_windows_prefix[MAX_PATH];
WCHAR wow64_native_dir[MAX_PATH];
WCHAR wow64_guest_dir[MAX_PATH];
#endif
...
struct reactos_syscall
{
    const char *name;
    BYTE args;
};

static const struct reactos_syscall reactos_syscalls[] =
{
...
static NTSTATUS WINAPI wow64_syscall_not_implemented( UINT *args )
{
...
#ifdef __REACTOS__
static void init_reactos_wow64_paths(void)
{
    const WCHAR *nt_system_root = (const WCHAR *)(ULONG_PTR)0x7ffe0030;

    wcscpy(wow64_windows_dir, L"\\??\\");
    wcscat(wow64_windows_dir, nt_system_root);
    wcscpy(wow64_windows_prefix, wow64_windows_dir);
    wcscat(wow64_windows_prefix, L"\\");
    wcscpy(wow64_native_dir, wow64_windows_dir);
    wcscat(wow64_native_dir, L"\\System32");
    wcscpy(wow64_guest_dir, wow64_windows_dir);
    wcscat(wow64_guest_dir, L"\\SysWOW64");
}

static void init_reactos_syscall_table(void)
{
    UINT i, j, missing = 0;

    for (i = 0; i < ARRAY_SIZE(reactos_syscalls); ++i)
    {
        reactos_syscall_thunks[i] = wow64_syscall_not_implemented;
        reactos_syscall_args[i] = reactos_syscalls[i].args;
        for (j = 0; j < ARRAY_SIZE(syscall_thunks); ++j)
        {
            if (!strcmp(reactos_syscalls[i].name, syscall_names[j]))
            {
                reactos_syscall_thunks[i] = syscall_thunks[j];
                break;
            }
        }
...
NTSTATUS WINAPI wow64_NtAlpcAcceptConnectPort( UINT *args )
{
    ...
    status = NtAlpcAcceptConnectPort( communication_port_ptr ? &communication_port : NULL, connection_port, flags, objattr_32to64( &attr, attr32 ), alpc_port_attributes_32to64( &port_attr, port_attr32 ), context, alpc_port_message_32to64( &msg, msg32 ? (sizeof(*msg) + msg32->DataLength) : 0, msg32, TRUE ), alpc_port_message_attributes_32to64( &msg_attr, msg_attr32, TRUE ), accept);
...
NTSTATUS WINAPI wow64_NtAlpcCreatePort( UINT *args )
{
    ...
    status = NtAlpcCreatePort( &handle, objattr_32to64( &attr, attr32 ), alpc_port_attributes_32to64( &port_attr, port_attr32 ) );
...
#else
    /* Load from the native system directory and keep it as the search path,
       so 64-bit imports (e.g. wow64win.dll -> win32u.dll) never resolve to
       the SysWOW64 copies through the 32-bit process default path. */
    const WCHAR *nt_system_root = (const WCHAR *)((const char *)0x7ffe0000 + 0x30); /* KUSER_SHARED_DATA.NtSystemRoot */
    WCHAR dir[MAX_PATH];

    swprintf( dir, MAX_PATH, L"%s\\system32", nt_system_root );
    swprintf( path, MAX_PATH, L"%s\\%s", dir, name );
    RtlInitUnicodeString( &str, path );
    if ((status = LdrLoadDll( dir, 0, &str, &module )))
#endif
...
static NTSTATUS create_cross_process_work_list( WOW64INFO *wow64info )
{
    ...
    status = NtMapViewOfSection( section, GetCurrentProcess(), (void **)&list, default_zero_bits, 0, NULL, &map_size, ViewShare, MEM_TOP_DOWN, PAGE_READWRITE );
    if (status)
    {
        NtClose( section );
        return status;
...
#ifdef __REACTOS__
            *(--stack) = 0;
            *(--stack) = pLdrSystemDllInitBlock->ntdll_handle;
            *(--stack) = 0;
            *(--stack) = pLdrSystemDllInitBlock->pLdrInitializeThunk;
#else
            *(--stack) = 0;
            *(--stack) = 0;
            *(--stack) = 0;
...
void WINAPI Wow64ProcessPendingCrossProcessItems(void)
{
    ...
    while (entry)
    {
        ...
        case CrossProcessPostVirtualAlloc:
            ...
            pBTCpuNotifyMemoryAlloc( (void *)entry->addr, entry->size, entry->args[0], entry->args[1], entry->id == CrossProcessPostVirtualAlloc, entry->args[2] );
            break;
        case CrossProcessPreVirtualFree:
        case CrossProcessPostVirtualFree:
            if (!pBTCpuNotifyMemoryFree) break;
            pBTCpuNotifyMemoryFree( (void *)entry->addr, entry->size, entry->args[0], entry->id == CrossProcessPostVirtualFree, entry->args[1] );
            break;
        case CrossProcessPreVirtualProtect:
        case CrossProcessPostVirtualProtect:
            if (!pBTCpuNotifyMemoryProtect) break;
            pBTCpuNotifyMemoryProtect( (void *)entry->addr, entry->size, entry->args[0], entry->id == CrossProcessPostVirtualProtect, entry->args[1] );
            break;
