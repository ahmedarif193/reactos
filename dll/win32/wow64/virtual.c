...
NTSTATUS WINAPI wow64_NtAllocateVirtualMemory( UINT *args )
{
    ...
    if (!is_current) send_cross_process_notification( process, CrossProcessPreVirtualAlloc, addr, size, 3, type, protect, 0 );
    ...
    if (!is_current) send_cross_process_notification( process, CrossProcessPostVirtualAlloc, addr, size, 3, type, protect, status );
...
NTSTATUS WINAPI wow64_NtCreateSectionEx( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtCreateSectionEx( &handle, access, objattr_32to64( &attr, attr32 ), size, protect, flags, file, params64, count );
...
NTSTATUS WINAPI wow64_NtFlushVirtualMemory( UINT *args )
{
    ...
    status = NtFlushVirtualMemory( process, (const void **)addr_32to64( &addr, addr32 ), size_32to64( &size, size32 ), iosb_32to64( &io, io32 ) );
    if (!status)
    {
        put_addr( addr32, addr );
        put_size( size32, size );
...
NTSTATUS WINAPI wow64_NtFreeVirtualMemory( UINT *args )
{
    ...
    if (!is_current) send_cross_process_notification( process, CrossProcessPreVirtualFree, addr, size, 2, type, 0 );
    ...
    if (!is_current) send_cross_process_notification( process, CrossProcessPostVirtualFree, addr, size, 2, type, status );
...
NTSTATUS WINAPI wow64_NtGetNlsSectionPtr( UINT *args )
{
    ...
    status = NtGetNlsSectionPtr( type, id, unknown, addr_32to64( &addr, addr32 ), size_32to64( &size, size32 ));
    if (!status)
    {
        put_addr( addr32, addr );
        put_size( size32, size );
...
NTSTATUS WINAPI wow64_NtLockVirtualMemory( UINT *args )
{
    ...
    status = NtLockVirtualMemory( process, addr_32to64( &addr, addr32 ), size_32to64( &size, size32 ), unknown );
    if (!status)
    {
        put_addr( addr32, addr );
        put_size( size32, size );
...
NTSTATUS WINAPI wow64_NtMapViewOfSection( UINT *args )
{
    ...
    status = NtMapViewOfSection( handle, process, addr_32to64( &addr, addr32 ), get_zero_bits( zero_bits ), commit, offset, size_32to64( &size, size32 ), inherit, alloc, protect );
...
NTSTATUS WINAPI wow64_NtMapViewOfSectionEx( UINT *args )
{
    ...
    NtCurrentTeb()->Tib.ArbitraryUserPointer = ULongToPtr( NtCurrentTeb32()->Tib.ArbitraryUserPointer );
    status = NtMapViewOfSectionEx( handle, process, addr_32to64( &addr, addr32 ), offset, size_32to64( &size, size32 ), alloc, protect, params64, count );
    if (NT_SUCCESS(status))
    {
        put_addr( addr32, addr );
        put_size( size32, size );
...
NTSTATUS WINAPI wow64_NtProtectVirtualMemory( UINT *args )
{
    ...
    if (!is_current) send_cross_process_notification( process, CrossProcessPreVirtualProtect, addr, size, 2, new_prot, 0 );
    ...
    if (!is_current) send_cross_process_notification( process, CrossProcessPostVirtualProtect, addr, size, 2, new_prot, status );
...
NTSTATUS WINAPI wow64_NtQueryVirtualMemory( UINT *args )
{
    ...
    case MemoryWineLoadUnixLibByName:
    {
        ...
        status = NtQueryVirtualMemory( handle, unicode_str_32to64( &str, str32 ), MemoryWineLoadUnixLibByNameWow64, ptr, len, &res_len );
        break;
    }
...
NTSTATUS WINAPI wow64_NtSetLdtEntries( UINT *args )
{
    ...
    LDT_ENTRY entry1, entry2;

    memcpy( &entry1, &entry1_low, sizeof(entry1_low) );
    memcpy( (char *)&entry1 + sizeof(entry1_low), &entry1_high, sizeof(entry1_high) );
    memcpy( &entry2, &entry2_low, sizeof(entry2_low) );
    memcpy( (char *)&entry2 + sizeof(entry2_low), &entry2_high, sizeof(entry2_high) );
    return NtSetLdtEntries( sel1, entry1, sel2, entry2 );
}
...
NTSTATUS WINAPI wow64_NtUnlockVirtualMemory( UINT *args )
{
    ...
    status = NtUnlockVirtualMemory( process, addr_32to64( &addr, addr32 ), size_32to64( &size, size32 ), unknown );
    if (!status)
    {
        put_addr( addr32, addr );
        put_size( size32, size );
