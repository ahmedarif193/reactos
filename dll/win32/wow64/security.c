...
NTSTATUS WINAPI wow64_NtAccessCheck( UINT *args )
{
    ...
    return NtAccessCheck( secdesc_32to64( &sd, sd32 ), handle, access, mapping, privs, retlen, access_granted, access_status );
}
...
NTSTATUS WINAPI wow64_NtAccessCheckAndAuditAlarm( UINT *args )
{
    ...
    return NtAccessCheckAndAuditAlarm( unicode_str_32to64( &subsystem, subsystem32 ), handle, unicode_str_32to64( &typename, typename32 ), unicode_str_32to64( &objname, objname32 ), secdesc_32to64( &sd, sd32 ), access, mapping, creation, access_granted, access_status, onclose );
}
...
NTSTATUS WINAPI wow64_NtAccessCheckByTypeAndAuditAlarm( UINT *args )
{
    ...
    return NtAccessCheckByTypeAndAuditAlarm( unicode_str_32to64( &subsystem, subsystem32 ), handle, unicode_str_32to64( &typename, typename32 ), unicode_str_32to64( &objname, objname32 ), secdesc_32to64( &sd, sd32 ), sid, access, audit_type, flags, objtypelist_32to64( &obj_list, list32 ), list_len, mapping, creation, access_granted, access_status, onclose );
}
...
NTSTATUS WINAPI wow64_NtCreateToken( UINT *args )
{
    ...
    status = NtCreateToken( &handle, access, objattr_32to64( &attr, attr32 ), type, luid, expire, token_user_32to64( &user, user32 ), token_groups_32to64( groups32 ), privs, token_owner_32to64( &owner, owner32 ), token_primary_group_32to64( &group, group32 ), token_default_dacl_32to64( &dacl, dacl32 ), source );
...
NTSTATUS WINAPI wow64_NtFilterToken( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtFilterToken( token, flags, token_groups_32to64( disable_sids32 ), privs, token_groups_32to64( restrict_sids32 ), &handle );
    put_handle( handle_ptr, handle );
    return status;
}
