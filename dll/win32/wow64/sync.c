...
static BOOL is_csr_api_port( const UNICODE_STRING *name )
{
    static const WCHAR suffix[] = L"\\ApiPort";
    const USHORT suffix_len = sizeof(suffix) - sizeof(suffix[0]);

    return name && name->Length >= suffix_len && !memcmp( name->Buffer + (name->Length - suffix_len) / sizeof(WCHAR), suffix, suffix_len );
}


static void put_port_message_header64( CSR_PORT_MESSAGE64 *out, const CSR_PORT_MESSAGE32 *in )
{
    out->Type = in->Type;
    out->DataInfoOffset = in->DataInfoOffset ? in->DataInfoOffset + sizeof(*out) - sizeof(*in) : 0;
    out->UniqueProcess = in->ClientId.UniqueProcess;
    out->UniqueThread = in->ClientId.UniqueThread;
    out->MessageId = in->MessageId;
    out->ClientViewSize = in->ClientViewSize;
}


static NTSTATUS put_port_message_header32( CSR_PORT_MESSAGE32 *out, const CSR_PORT_MESSAGE64 *in )
{
    if (in->DataInfoOffset && in->DataInfoOffset < sizeof(*in)) return STATUS_INVALID_PARAMETER;

    out->Type = in->Type;
    out->DataInfoOffset = in->DataInfoOffset ? in->DataInfoOffset - sizeof(*in) + sizeof(*out) : 0;
    out->ClientId.UniqueProcess = in->UniqueProcess;
    out->ClientId.UniqueThread = in->UniqueThread;
    out->MessageId = in->MessageId;
    out->ClientViewSize = in->ClientViewSize;
...
static NTSTATUS lpc_message_32to64( CSR_PORT_MESSAGE64 *out, const CSR_PORT_MESSAGE32 *in )
{
    ULONG payload_size;

    if (in->TotalLength < sizeof(*in) || in->DataLength + sizeof(*in) > in->TotalLength) return STATUS_INVALID_PARAMETER;
    if (in->TotalLength + sizeof(*out) - sizeof(*in) > WOW64_LPC_MAX_MESSAGE_LENGTH) return STATUS_PORT_MESSAGE_TOO_LONG;
    payload_size = in->TotalLength - sizeof(*in);
    memset( out, 0, in->TotalLength + sizeof(*out) - sizeof(*in) );
    put_port_message_header64( out, in );
    out->DataLength = in->DataLength;
    out->TotalLength = in->TotalLength + sizeof(*out) - sizeof(*in);
    memcpy( out + 1, in + 1, payload_size );
    return STATUS_SUCCESS;
}


static NTSTATUS lpc_message_64to32( CSR_PORT_MESSAGE32 *out, const CSR_PORT_MESSAGE64 *in )
{
    ULONG payload_size;
    NTSTATUS status;

    if (in->TotalLength < sizeof(*in) || in->DataLength + sizeof(*in) > in->TotalLength) return STATUS_INVALID_PARAMETER;
    if (in->TotalLength > WOW64_LPC_MAX_MESSAGE_LENGTH) return STATUS_PORT_MESSAGE_TOO_LONG;
    payload_size = in->TotalLength - sizeof(*in);
    if ((status = put_port_message_header32( out, in ))) return status;
    out->DataLength = in->DataLength;
    out->TotalLength = in->TotalLength - sizeof(*in) + sizeof(*out);
    memcpy( out + 1, in + 1, payload_size );
    return STATUS_SUCCESS;
}
...
static NTSTATUS csr_client_connect_32to64( CSR_API_MESSAGE64 *out, const CSR_API_MESSAGE32 *in )
{
    ...
    const USHORT total_length = FIELD_OFFSET(CSR_API_MESSAGE64, Data) + sizeof(out->Data.CsrClientConnect);

    if (in->Header.TotalLength < FIELD_OFFSET(CSR_API_MESSAGE32, Data) + sizeof(in->Data.CsrClientConnect)) return STATUS_INVALID_PARAMETER;
    if (in->ApiNumber != 0) return STATUS_NOT_IMPLEMENTED;
    memset( out, 0, sizeof(*out) );
    put_port_message_header64( &out->Header, &in->Header );
    out->Header.DataLength = data_length;
    out->Header.TotalLength = total_length;
...
static NTSTATUS csr_client_connect_64to32( CSR_API_MESSAGE32 *out, const CSR_API_MESSAGE64 *in )
{
    NTSTATUS status;

    if ((status = put_port_message_header32( &out->Header, &in->Header ))) return status;
    out->Header.DataLength = FIELD_OFFSET(CSR_API_MESSAGE32, Data) - sizeof(out->Header) + sizeof(out->Data.CsrClientConnect);
    out->Header.TotalLength = FIELD_OFFSET(CSR_API_MESSAGE32, Data) + sizeof(out->Data.CsrClientConnect);
    out->CsrCaptureData = in->CsrCaptureData;
    out->ApiNumber = in->ApiNumber;
    out->Status = in->Status;
    out->Reserved = in->Reserved;
...
NTSTATUS WINAPI wow64_NtCreateSymbolicLinkObject( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtCreateSymbolicLinkObject( &handle, access, objattr_32to64( &attr, attr32 ), unicode_str_32to64( &target, target32 ));
    put_handle( handle_ptr, handle );
    return status;
}
...
NTSTATUS WINAPI wow64_NtDuplicateObject( UINT *args )
{
    ...
    status = NtDuplicateObject( source_process, source_handle, dest_process, &handle, access, attributes, options );
...
NTSTATUS WINAPI wow64_NtRequestWaitReplyPort( UINT *args )
{
    ...
    WOW64_LPC_BUFFER request_buffer, reply_buffer;
    CSR_PORT_MESSAGE32 *request32 = (CSR_PORT_MESSAGE32 *)msg_in;
    CSR_PORT_MESSAGE32 *reply32 = (CSR_PORT_MESSAGE32 *)msg_out;
    CSR_PORT_MESSAGE64 *request64 = (CSR_PORT_MESSAGE64 *)request_buffer.Data;
    ...
    status = NtRequestWaitReplyPort( handle, (LPC_MESSAGE *)request64, (LPC_MESSAGE *)reply64 );
    if (NT_SUCCESS(status)) status = lpc_message_64to32( reply32, reply64 );
    return status;
}
...
NTSTATUS WINAPI wow64_NtSecureConnectPort( UINT *args )
{
    ...
    UNICODE_STRING32 *name32 = get_ptr( &args );
    SECURITY_QUALITY_OF_SERVICE *qos = get_ptr( &args );
    LPC_SECTION_WRITE32 *write32 = get_ptr( &args );
    SID *sid = get_ptr( &args );
    LPC_SECTION_READ32 *read32 = get_ptr( &args );
    ULONG *max_len = get_ptr( &args );
    void *info = get_ptr( &args );
    ULONG *info_len = get_ptr( &args );
    ...
    *handle_ptr = 0;
    if (write32)
    {
        ...
        write.SectionHandle = LongToHandle( write32->SectionHandle );
    ...
    name_ptr = unicode_str_32to64( &name, name32 );
    status = NtSecureConnectPort( &handle, name_ptr, qos, write32 ? &write : NULL, sid, read32 ? &read : NULL, max_len, info, info_len );
    if (NT_SUCCESS(status))
    {
        put_handle( handle_ptr, handle );
...
NTSTATUS WINAPI wow64_NtSetInformationJobObject( UINT *args )
{
    ...
    case JobObjectBasicLimitInformation:   /* JOBOBJECT_BASIC_LIMIT_INFORMATION */
        if (len == sizeof(JOBOBJECT_BASIC_LIMIT_INFORMATION32))
        {
            ...
            return NtSetInformationJobObject( handle, class, job_basic_limit_info_32to64( &info, ptr ), sizeof(info) );
        }
    ...
    case JobObjectExtendedLimitInformation:   /* JOBOBJECT_EXTENDED_LIMIT_INFORMATION */
        if (len == sizeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION32))
        {
            ...
            return NtSetInformationJobObject( handle, class, job_basic_limit_info_32to64( &info.BasicLimitInformation, &info32->BasicLimitInformation ), sizeof(info) );
        }
        else return STATUS_INVALID_PARAMETER;
...
NTSTATUS WINAPI wow64_NtSetTimer( UINT *args )
{
    ...
    return NtSetTimer( handle, when, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), resume, period, state );
}
...
static NTSTATUS get_image_machine( HANDLE handle, USHORT *machine )
{
    ...
    offset.QuadPart = 0;
    status = NtReadFile( handle, NULL, NULL, NULL, &iosb, &dos_hdr, sizeof(dos_hdr), &offset, NULL );
    if (!status)
    {
        offset.QuadPart = dos_hdr.e_lfanew;
        status = NtReadFile( handle, NULL, NULL, NULL, &iosb, &nt_hdr, FIELD_OFFSET(IMAGE_NT_HEADERS, OptionalHeader), &offset, NULL );
        if (!status)
...
NTSTATUS WINAPI wow64_NtCreateTransaction( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtCreateTransaction( &handle, access, objattr_32to64( &attr, attr32 ), guid, tm, options, isol_level, isol_flags, timeout, unicode_str_32to64( &desc, desc32 ));
    put_handle( handle_ptr, handle );
