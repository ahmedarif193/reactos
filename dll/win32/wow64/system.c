...
NTSTATUS WINAPI wow64_NtQuerySystemEnvironmentValueEx( UINT *args )
{
    ...
    return NtQuerySystemEnvironmentValueEx( unicode_str_32to64( &str, str32 ), vendor, buffer, retlen, attrib );
}
    ...
    case SystemLogicalProcessorInformationEx:  /* SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX */
    {
        ...
        if (!status)
        {
            for (pos = pos32 = 0; pos < size; pos += ex->Size, pos32 += size32)
            {
                ...
                case RelationProcessorCore:
                case RelationProcessorPackage:
                    size32 = offsetof( SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX32, Processor.GroupMask[ex->Processor.GroupCount] );
                    break;
                case RelationNumaNode:
                ...
                case RelationCache:
                    ...
                    break;
                case RelationGroup:
                    size32 = offsetof( SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX32, Group.GroupInfo[ex->Group.MaximumGroupCount] );
                    break;
                default:
                    size32 = 0;
                    continue;
