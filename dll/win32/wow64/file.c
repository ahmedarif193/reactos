...
#ifdef __REACTOS__
    UNICODE_STRING windows, system32;
#else
...
#ifdef __REACTOS__
    RtlInitUnicodeString(&windows, wow64_windows_dir);
    RtlInitUnicodeString(&system32, wow64_native_dir);
#endif
    InitializeObjectAttributes( &attr, &windows, OBJ_CASE_INSENSITIVE, 0, NULL );
    if (!NtOpenFile( &handle, SYNCHRONIZE | FILE_LIST_DIRECTORY, &attr, &io, FILE_SHARE_READ, FILE_SYNCHRONOUS_IO_NONALERT | FILE_OPEN_FOR_BACKUP_INTENT | FILE_DIRECTORY_FILE ))
    {
...
#ifdef __REACTOS__
    const WCHAR *windirW = wow64_windows_prefix;
#else
...
NTSTATUS WINAPI wow64_NtCreateFile( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtCreateFile( &handle, access, objattr_32to64_redirect( &attr, attr32 ), iosb_32to64( &io, io32 ), alloc_size, attributes, sharing, disposition, options, ea_buffer, ea_length );
...
NTSTATUS WINAPI wow64_NtCreateMailslotFile( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtCreateMailslotFile( &handle, access, objattr_32to64( &attr, attr32 ), iosb_32to64( &io, io32 ), options, quota, msg_size, timeout );
    put_handle( handle_ptr, handle );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtCreateNamedPipeFile( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtCreateNamedPipeFile( &handle, access, objattr_32to64( &attr, attr32 ), iosb_32to64( &io, io32 ), sharing, dispo, options, pipe_type, read_mode, completion_mode, max_inst, inbound_quota, outbound_quota, timeout );
    put_handle( handle_ptr, handle );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtDeviceIoControlFile( UINT *args )
{
    ...
    status = NtDeviceIoControlFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), code, in_buf, in_len, out_buf, out_len );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtFsControlFile( UINT *args )
{
    ...
    status = NtFsControlFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), code, in_buf, in_len, out_buf, out_len );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtLockFile( UINT *args )
{
    ...
    status = NtLockFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), offset, count, key, dont_wait, exclusive );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtNotifyChangeDirectoryFile( UINT *args )
{
    ...
    status = NtNotifyChangeDirectoryFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), buffer, len, filter, subtree );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtOpenFile( UINT *args )
{
    ...
    *handle_ptr = 0;
    status = NtOpenFile( &handle, access, objattr_32to64_redirect( &attr, attr32 ), iosb_32to64( &io, io32 ), sharing, options );
    put_handle( handle_ptr, handle );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtQueryDirectoryFile( UINT *args )
{
    ...
    status = NtQueryDirectoryFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), buffer, len, class, single_entry, unicode_str_32to64( &mask, mask32 ), restart_scan );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtQueryEaFile( UINT *args )
{
    ...
    status = NtQueryEaFile( handle, iosb_32to64( &io, io32 ), buffer, len, single_entry, list, list_len, index, restart );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtReadFile( UINT *args )
{
    ...
    status = NtReadFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), buffer, len, offset, key );
...
NTSTATUS WINAPI wow64_NtReadFileScatter( UINT *args )
{
    ...
    status = NtReadFileScatter( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), segments, len, offset, key );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtWriteFile( UINT *args )
{
    ...
    status = NtWriteFile( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), buffer, len, offset, key );
    put_iosb( io32, &io );
    return status;
}
...
NTSTATUS WINAPI wow64_NtWriteFileGather( UINT *args )
{
    ...
    status = NtWriteFileGather( handle, event, apc_32to64( apc ), apc_param_32to64( apc, apc_param ), iosb_32to64( &io, io32 ), segments, len, offset, key );
    put_iosb( io32, &io );
    return status;
}
