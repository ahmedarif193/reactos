...
static void rsym_decode_entry(const BYTE* raw, unsigned address_size, rsym_entry_t* entry)
{
    if (address_size == 64)
    {
        ROSSYM_ENTRY64 raw_entry;
...
BOOL rsym_parse(struct module* module, DWORD64 load_offset,
                unsigned address_size, const void* rsym_ptr, unsigned rsymlen)
{
    ...
    EntrySize = address_size == 64 ? sizeof(ROSSYM_ENTRY64) : sizeof(ROSSYM_ENTRY32);
    ...
    for (Index = 0; Index < EntryCount; Index++)
    {
        ...
        rsym_decode_entry(First + Index * EntrySize, address_size, &Entry);
