   ...
   PPDEV ppdev)
{
   ...
   if (EngDeviceIoControl(ppdev->hDriver,
                          ...
                          NULL,
                          0,
                          &ppdev->HwPointerCapabilities,
                          sizeof(ppdev->HwPointerCapabilities),
                          &Returned) != 0)
   {
      return TRUE;
   }
   ...
   ppdev->HwPointerAttributesSize =
      FIELD_OFFSET(VIDEO_POINTER_ATTRIBUTES, Pixels) + PixelBytes;
   ppdev->HwPointerAttributes =
      EngAllocMem(FL_ZERO_MEMORY, ppdev->HwPointerAttributesSize, ALLOC_TAG);
   ...
   OUT PULONG Height)
{
   ...
   PixelBytes = ppdev->HwPointerAttributesSize -
   ...
   for (y = 0; y < SourceHeight; ++y)
   {
      ...
      else
      {
         for (x = 0; x < SourceWidth; ++x)
         {
            ...
            if (AndMask && !XorMask)
               DestinationRow[x] = 0x00000000;   /* transparent  */
            else if (!AndMask && !XorMask)
               DestinationRow[x] = 0xFF000000;   /* opaque black  */
            else if (!AndMask && XorMask)
               DestinationRow[x] = 0xFFFFFFFF;   /* opaque white  */
            else
               /*
                * AND=XOR=1 means "invert the destination". A hardware overlay
                * plane cannot XOR the framebuffer, so approximate the invert
                * pixels as opaque black. This keeps invert-based cursors (the
                * I-beam, resize and crosshair shapes) on the hardware pointer
                * plane instead of dropping to the software pointer, whose
                * save/restore blits corrupt the desktop as the cursor moves.
                */
   ...
   IN FLONG fl)
{
   ...
   Attributes->WidthInBytes =
      ppdev->HwPointerCapabilities.MaxWidth * sizeof(ULONG);
...
{
   if (pso != NULL &&
       FrameBufferSetHardwarePointerShape((PPDEV)pso->dhpdev,
                                          psoMask,
                                          psoColor,
                                          xHot,
                                          yHot,
   ...
   /*
    * Hardware pointer not usable for this shape: decline so the GDI engine
    * installs the software pointer exactly once. (Returning EngSetPointerShape's
    * result here would make IntEngSetPointerShape set it up a second time.)
    */
   return SPS_DECLINE;
}
