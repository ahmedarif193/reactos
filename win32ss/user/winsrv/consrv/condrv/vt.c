                   ...
                   ULONG Length)
{
    ...
    while (Ptr < End)
    {
        ...
        if (ColorStart < Ptr && Index < 16)
        {
            ...
            {
                VtSendOscPaletteResponse(Console, Index, Cons->Colors[PaletteIndex]);
                Handled = TRUE;
            }
            else if (VtOscParseRgb(ColorStart, ColorLength, &Parsed))
            {
                Cons->Colors[PaletteIndex] = Parsed;
                Handled = TRUE;
                Updated = TRUE;
            }
        }
...
static USHORT
VtAnsiIndexToFgAttr(UCHAR Index)
{
    USHORT Attr = VtAnsiFgMap[Index & 0x7];
    if (Index & 0x8) Attr |= FOREGROUND_INTENSITY;
    return Attr;
}

static USHORT
VtAnsiIndexToBgAttr(UCHAR Index)
{
    USHORT Attr = VtAnsiBgMap[Index & 0x7];
    if (Index & 0x8) Attr |= BACKGROUND_INTENSITY;
    return Attr;
}

static USHORT
VtConsoleIndexToFgAttr(UCHAR Index)
{
    return (USHORT)(Index & 0x0F);
}

static USHORT
VtConsoleIndexToBgAttr(UCHAR Index)
{
    return (USHORT)((Index & 0x0F) << 4);
}
            ...
            case 48:
            {
                ...
                if (i + 1 < Count)
                {
                    ...
                    if (mode == 2 && (i + 3) < Count)
                    {
                        ...
                        {
                            Attr &= ~FG_ATTR_MASK;
                            Attr |= VtConsoleIndexToFgAttr(nearest);
                        ...
                        {
                            Attr &= ~BG_ATTR_MASK;
                            Attr |= VtConsoleIndexToBgAttr(nearest);
                    ...
                    else if (mode == 5 && (i + 1) < Count)
                    {
                        ...
                        if (idx < 16)
                        {
                            ...
                            {
                                Attr &= ~FG_ATTR_MASK;
                                Attr |= VtAnsiIndexToFgAttr((UCHAR)idx);
                                ScreenBuffer->VtState.UseRgbForeground = FALSE;
                            }
                            else
                            {
                                Attr &= ~BG_ATTR_MASK;
                                Attr |= VtAnsiIndexToBgAttr((UCHAR)idx);
                                ScreenBuffer->VtState.UseRgbBackground = FALSE;
                            }
                        }
                        else
