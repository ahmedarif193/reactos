...
static VOID
IntCompositionSyncEscape(_In_ PDC pdcScreen, _In_ LONG value)
{
    PPDEVOBJ ppdev = pdcScreen->ppdev;
    ...
    ppdev->DriverFunctions.Escape(&ppdev->pSurface->SurfObj,
                                  CDD_ESCAPE_COMPOSITION_SYNC,
                                  sizeof(LONG), &value, 0, NULL);
}
...
/*
 * The painting thread finished its whole batch (cPaintsReady hit 0). All of
 * its windows are quiescent, so force the bracket counter to 0 (self-heals any
 * acquire/release drift that would otherwise defer the window until the stale
 * guard) and wake dwm for one coalesced compose per batch, not one per
 * control.
 */
VOID
IntCompositionPaintBatchDone(_In_ PWND Wnd)
{
    REDIRECT_ENTRY *e;
...
IntCompositionDwmGetFrame(_In_ PVOID pUser)
{
    ...
    BOOL dirty;
    LONG dmgL = 0, dmgT = 0, dmgR = 0, dmgB = 0;
    BOOL bHaveDmg = FALSE;
    ...
    _SEH2_TRY
    {
        ...
        for (i = 0; i < n && count < DWM_MAX_WINDOWS; i++)
        {
            ...
            wins[count].Damaged = e->Damaged ? 1 : 0;
            {
                ...
                COLORREF key = 0;
                DWORD lf = 0;
                if (IntCompositionGetLayered(w, &alpha, &key, &lf))
                {
                ...
                {
                    wins[count].Alpha = 255;
                    wins[count].ColorKey = 0;
                    wins[count].LayerFlags = 0;
                }
            }
            count++;
            ...
            if (e->Damaged)
            {
                if (!bHaveDmg)
                {
                    dmgL = w->rcWindow.left; dmgT = w->rcWindow.top;
                    dmgR = w->rcWindow.right; dmgB = w->rcWindow.bottom;
                    bHaveDmg = TRUE;
                }
                else
                {
                    if (w->rcWindow.left   < dmgL) dmgL = w->rcWindow.left;
                    if (w->rcWindow.top    < dmgT) dmgT = w->rcWindow.top;
                    if (w->rcWindow.right  > dmgR) dmgR = w->rcWindow.right;
                    if (w->rcWindow.bottom > dmgB) dmgB = w->rcWindow.bottom;
                }
            }
            e->Damaged = FALSE;
        }
        ...
        hdr->DmgL = dmgL;
        hdr->DmgT = dmgT;
        hdr->DmgR = bHaveDmg ? dmgR : 0;
        hdr->DmgB = bHaveDmg ? dmgB : 0;
    }
    _SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
    {
        Status = _SEH2_GetExceptionCode();
...
IntCompositionRegisterVblank(_In_opt_ PKEVENT Event)
{
    ...
    {
        ppdev->DriverFunctions.Escape(&ppdev->pSurface->SurfObj,
                                      CDD_ESCAPE_REGISTER_VBLANK,
                                      sizeof(payload), &payload, 0, NULL);
    }
    DC_UnlockDc(pdcScreen);
}
...
IntCompositionDwmAttach(_In_ PVOID pUser)
{
    ...
    /* Tear down the previous attachment: the display path must stop
     * signaling the vblank event before its reference goes away. */
...
IntCompositionDwmSync(_In_ LONG value)
{
    ...
    if (pdcScreen == NULL)
        return;
    IntCompositionSyncEscape(pdcScreen, value);
    DC_UnlockDc(pdcScreen);
}
    ...
    if (ClipChildren)
    {
        ...
        while (CurrentWindow != NULL)
        {
                ...
                !(CurrentWindow->ExStyle & WS_EX_TRANSPARENT))
            {
                ...
                if (ClipRgn != NULL)
                {
                    if (CurrentWindow->hrgnClip &&
                        !(CurrentWindow->style & WS_MINIMIZE))
                    {
                        PREGION ChildClipRgn = REGION_LockRgn(CurrentWindow->hrgnClip);
                        if (ChildClipRgn)
                        {
                            REGION_bOffsetRgn(ClipRgn,
                                              -CurrentWindow->rcWindow.left,
                                              -CurrentWindow->rcWindow.top);
                            IntGdiCombineRgn(ClipRgn, ClipRgn, ChildClipRgn, RGN_AND);
                            REGION_bOffsetRgn(ClipRgn,
                                              CurrentWindow->rcWindow.left,
                                              CurrentWindow->rcWindow.top);
                            REGION_UnlockRgn(ChildClipRgn);
                        }
                    }
                    IntGdiCombineRgn(VisRgn, VisRgn, ClipRgn, RGN_DIFF);
                    REGION_Delete(ClipRgn);
