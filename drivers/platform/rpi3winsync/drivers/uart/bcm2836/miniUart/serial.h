...
UCHAR SerialReadRegisterUChar(_In_ UCHAR* x)
{
    ...
    uchReturnValue=(UCHAR)(READ_REGISTER_ULONG(pulReg));
...
#define READ_MINIUARTRXTX_ENABLE(Extension, BaseAddress)           \
    (Extension->SerialReadUChar((BaseAddress)+EXTRA_CONTROL_REG))

#define READ_EXTRA_STATUS(BaseAddress)           \
    (READ_REGISTER_ULONG((volatile ULONG*)((BaseAddress)+EXTRA_STATUS_REG)))

#define READ_EXTRA_BAUD(BaseAddress)           \
    (READ_REGISTER_USHORT((volatile USHORT*)((BaseAddress)+BAUD_DIVISOR_REG)))
...
#define READ_BAUD_DIVISOR(Extension, BaseAddress)          \
    (Extension->SerialReadUChar((BaseAddress + BAUD_DIVISOR_REG)))
...
{                     
    ...
    for(lCnt=0; lCnt<(LONG)TxN;lCnt++)
    {
        ulByteOut=*(TransmitChars+lCnt);
        WRITE_REGISTER_ULONG(pFifoTxReg,(ULONG)ulByteOut);
    };
    return;
}
...
//
// This macro reads aux interrupt status register
//
// Arguments:
//
// BaseAddress - A pointer to the address from which the hardware
...
//
//
#define READ_AUXINTERRUPT_STATUS(Extension, BaseAddress)  \
