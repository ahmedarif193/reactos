    ...
    _In_ BOOLEAN NotifyHub)
{
    ...
    /* If the device disconnected, proactively disable every slot hanging off
     * this root port (the directly attached device and, when that device is a
     * hub, all devices behind it) to avoid stale state. */
    if ((PortSc & XHCI_PORTSC_CCS) == 0)
    {
        ...
        for (SlotIndex = 1; SlotIndex <= Extension->MaxSlots && SlotIndex <= XHCI_MAX_SLOTS; SlotIndex++)
        {
            ...
            if (!Slot->InUse || Slot->DisablePending || Slot->RootPortNumber != (UCHAR)PortId)
                continue;

            Slot->DisablePending = TRUE;
            DPRINT1("usbxhci: port %u disconnect, disabling slot %u (route %05lx)\n", PortId, Slot->SlotId, Slot->RouteString);
