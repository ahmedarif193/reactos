...
/*
 * A contiguous user request already has a locked MDL and a spare lower stack
 * location. Stop its completion at this driver so metadata state can be
 * updated before the filesystem completes the original request.
 */
static
NTSTATUS
NTAPI
ExFatOriginalIrpCompletion(
    PDEVICE_OBJECT DeviceObject,
    PIRP Irp,
    PVOID Context)
{
    ...
    PLARGE_INTEGER Offset)
{
    ...
    Irp->IoStatus = IoContext.IoStatus;

    if (NT_SUCCESS(IoContext.IoStatus.Status) &&
    ...
    PLARGE_INTEGER Offset)
{
    ...
    /* The caller's locked MDL already describes this transfer. */
    if (MdlOffset == 0 &&
        SourceMdl->Next == NULL &&
        Length <= MmGetMdlByteCount(SourceMdl))
    {
        return ExFatSubmitDeviceIo(DeviceObject,
                                   MajorFunction,
                                   SourceMdl,
    ...
    ULONG Length)
{
    ...
    if (OriginalIrp &&
        SourceMdl &&
        OriginalIrp->MdlAddress == SourceMdl &&
        MdlOffset == 0 &&
        Length == MmGetMdlByteCount(SourceMdl))
    {
        Status = ExFatSubmitOriginalIrp(Vcb->StorageDevice,
                                        OriginalIrp,
                                        MajorFunction,
                                        Length,
                                        &DeviceOffset);
    ...
    if (MajorFunction == IRP_MJ_WRITE)
    {
        /* Drop any blocks a concurrent reader cached during our write. */
        ExFatInvalidateSectorCacheRange(Vcb, Sector, Sectors);
    }
