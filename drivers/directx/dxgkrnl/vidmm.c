    ...
    for (i = 0; i < SegmentCount; i++)
    {
        ...
        if (UsingSeg4)
        {
            PDXGK_SEGMENTDESCRIPTOR4 Desc =
                (PDXGK_SEGMENTDESCRIPTOR4)(DescBuffer + i * DescStride);
        ...
        else if (UsingSeg3)
        {
            PDXGK_SEGMENTDESCRIPTOR3 Desc =
                (PDXGK_SEGMENTDESCRIPTOR3)(DescBuffer + i * DescStride);
        ...
        else
        {
            PDXGK_SEGMENTDESCRIPTOR Desc =
                (PDXGK_SEGMENTDESCRIPTOR)(DescBuffer + i * DescStride);
    ...
    _Out_ PVOID                *OutVa)
{
    ...
    *OutVa = UserVa;
    ...
    {
        static LONG UserMapTrace = 0;

        if (InterlockedIncrement(&UserMapTrace) <= 16)
            DPRINT1("VidMm usermap #%ld h=0x%X va=%p resident=%u seg=%lu pa=%I64x cache=%d\n",
                    UserMapTrace, Allocation->Handle, UserVa,
                    Allocation->Resident, Allocation->SegmentId,
                    Allocation->PhysicalAddress.QuadPart, (int)CacheType);
    }
