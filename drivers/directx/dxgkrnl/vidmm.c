    ...
    _In_ PDXGKRNL_ADAPTER Adapter)
{
     ...
     * Skip VidMm initialization for DODs entirely.
     *
     * A DOD uses the KMDDOD_INITIALIZATION_DATA layout (the .dod union
     * member), which has NO DxgkDdiCreateAllocation field, so the .s union
     * view of that offset reads an unrelated (non-NULL) DOD callback.  We
     * must therefore test IsDisplayOnlyDriver FIRST — inspecting
     * InitData.s.DxgkDdiCreateAllocation on a DOD is meaningless and would
     * (wrongly) drive a segment query that fails with STATUS_NOT_SUPPORTED.
     */
    DPRINT("DxgkVidMmInitializeAdapter: DOD=%d\n",
