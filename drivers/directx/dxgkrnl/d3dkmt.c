    ...
    _Inout_ D3DKMT_CREATEPAGINGQUEUE *pData)
{
    ...
    DPRINT1("DxgkCreatePagingQueue: RETURN hPagingQueue=0x%x\n", pData->hPagingQueue);
    ...
    _In_ PIO_STACK_LOCATION Stack)
{
    ...
    switch (IoControlCode)
    {
        ...
        {
            static LONG CreateAllocTrace = 0;
            LONG TraceSlot;

            if (InputLength < sizeof(D3DKMT_CREATEALLOCATION) || SystemBuffer == NULL)
                return STATUS_BUFFER_TOO_SMALL;

            TraceSlot = InterlockedIncrement(&CreateAllocTrace);
            if (TraceSlot <= 8)
                DPRINT1("D3DKMT CREATEALLOC #%ld enter\n", TraceSlot);
            Status = DxgkCreateAllocation((D3DKMT_CREATEALLOCATION *)SystemBuffer);
            if (TraceSlot <= 8)
                DPRINT1("D3DKMT CREATEALLOC #%ld exit 0x%08lx\n", TraceSlot, Status);
            if (NT_SUCCESS(Status))
        ...
        case IOCTL_D3DKMT_LOCK:
        {
            ...
            LockSlot = InterlockedIncrement(&LockTrace);
            if (LockSlot <= 8)
                DPRINT1("D3DKMT LOCK #%ld enter hAlloc=0x%x\n",
                        LockSlot, pLock->hAllocation);
            ...
            if (LockSlot <= 8)
                DPRINT1("D3DKMT LOCK #%ld exit va=%p\n", LockSlot, LockVa);
        ...
        case IOCTL_D3DKMT_MAPGPUVIRTUALADDRESS:
        {
            ...
            {
                static LONG MapTrace = 0;

                if (InterlockedIncrement(&MapTrace) <= 8)
                    DPRINT1("D3DKMT MAP: ioctl reached hAlloc=0x%x pages=%u ctx=%p\n",
                            pMap->hAllocation, pMap->SizeInPages, Context);
            }
            ...
            if (pMap->hAllocation != 0)
            {
                ...
                if (MapAlloc == NULL)
                {
                    DPRINT1("D3DKMT MAP: MapAlloc NULL hAlloc=0x%x\n", pMap->hAllocation);
                    return STATUS_INVALID_HANDLE;
                }
                ...
                {
                    static LONG MapExitTrace = 0;

                    if (InterlockedIncrement(&MapExitTrace) <= 16)
                        DPRINT1("D3DKMT MAP exit hAlloc=0x%x va=%I64x\n",
                                pMap->hAllocation,
                                (ULONG64)pMap->VirtualAddress);
                }
