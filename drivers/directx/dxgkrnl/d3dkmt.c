    ...
    _Inout_ D3DKMT_ENUMADAPTERS2 *pEnumAdapters2)
{
    ...
    /* If caller's buffer is too small, return required count */
    if (pEnumAdapters2->pAdapters == NULL || pEnumAdapters2->NumAdapters < Started)
    {
        pEnumAdapters2->NumAdapters = Started;
        if (pEnumAdapters2->pAdapters == NULL && Started == 0)
            return STATUS_SUCCESS;
    ...
    _In_ PIO_STACK_LOCATION Stack)
{
    ...
    switch (IoControlCode)
    {
        ...
        {
            /*
             * D3DKMT_PRESENT grows with DXGKDDI_INTERFACE_VERSION (WIN8 /
             * WDDM1_3 / WDDM2_0 append fields).  dxgkrnl is built at WDDM2_0
             * while the win32k bridge stubs may compile a smaller (Vista/Win7)
             * D3DKMT_PRESENT, so requiring the full WDDM2_0 sizeof rejects a
             * perfectly valid present with STATUS_BUFFER_TOO_SMALL.
             *
             * DxgkPresent only reads fields up to and including Flags (all in
             * the Vista baseline), so require just that much.  This makes the
             * present IOCTL ABI-tolerant across interface versions.
             */
        ...
        {
            D3DKMT_CREATESYNCHRONIZATIONOBJECT2 *pCreate2;
            D3DKMT_CREATESYNCHRONIZATIONOBJECT Create1;

            if (InputLength < sizeof(D3DKMT_CREATESYNCHRONIZATIONOBJECT2) || SystemBuffer == NULL)
                return STATUS_BUFFER_TOO_SMALL;

            pCreate2 = (D3DKMT_CREATESYNCHRONIZATIONOBJECT2 *)SystemBuffer;
            RtlZeroMemory(&Create1, sizeof(Create1));
            Create1.hDevice = pCreate2->hDevice;
            Create1.Info.Type = pCreate2->Info.Type;

            switch (pCreate2->Info.Type)
            {
                case D3DDDI_SYNCHRONIZATION_MUTEX:
                    Create1.Info.SynchronizationMutex.InitialState =
                        pCreate2->Info.SynchronizationMutex.InitialState;
                    break;
                case D3DDDI_SEMAPHORE:
                    Create1.Info.Semaphore.MaxCount =
                        pCreate2->Info.Semaphore.MaxCount;
                    Create1.Info.Semaphore.InitialCount =
                        pCreate2->Info.Semaphore.InitialCount;
                    break;
                default:
                    return STATUS_NOT_SUPPORTED;
            }
            ...
            if (NT_SUCCESS(Status))
            {
                pCreate2->hSyncObject = Create1.hSyncObject;
        ...
        {
            /* Wait sync v2: not yet implemented */
            return STATUS_NOT_IMPLEMENTED;
        }

        case IOCTL_D3DKMT_SIGNALSYNCHRONIZATIONOBJECT2:
        {
            /* Signal sync v2: not yet implemented */
            return STATUS_NOT_IMPLEMENTED;
        }
