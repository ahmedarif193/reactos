...
/* ========================================================================
 * DxgkpEnsureShadowFb  (private)
 *
 * Ensures the adapter has a shadow framebuffer sized to the committed
 * display mode.  Normally the shadow FB is created by the GDI display
 * path's IOCTL_VIDEO_MAP_VIDEO_MEMORY, but when the GDI desktop lives on
 * a different display device (e.g. the firmware GOP framebuffer) nothing
 * maps the WDDM display, so the DOD present path creates it on demand.
 *
 * IRQL: PASSIVE_LEVEL
 * ====================================================================== */
static NTSTATUS
    ...
    _In_ PDXGKRNL_ADAPTER Adapter)
{
    ...
    if (Adapter->ShadowFb != NULL)
        return STATUS_SUCCESS;

    Width  = Adapter->CommittedWidth;
    Height = Adapter->CommittedHeight;
    if (Width == 0 || Height == 0)
        return STATUS_DEVICE_NOT_READY;

    if (Width > (MAXULONG / 4) || Height > (MAXULONG / (Width * 4)))
        return STATUS_INVALID_PARAMETER;
    ...
    Adapter->ShadowFb      = FbVa;
    Adapter->ShadowFbPitch = Pitch;
    Adapter->ShadowFbSize  = FbSize;

    DXGKRNL_TRACE("DxgkpEnsureShadowFb: on-demand shadow FB %lux%lu pitch=%lu "
                  "size=0x%lX at %p\n",
                  Width, Height, Pitch, FbSize, FbVa);

    return STATUS_SUCCESS;
}

/* ========================================================================
 * DxgkpCopyPresentSourceToShadow  (private)
 *
 * Copies the CPU-visible content of a present source allocation (e.g.
 * dwm.exe's locked composition allocation) into the adapter shadow
 * framebuffer.  The DOD miniport then blts the shadow FB to the scanout,
 * so this is the bridge that carries a user-mode D3DKMTPresent of a
 * COMPLETE composed frame onto the display — the tear-free flip.
 *
 * Copy is row-by-row to honour differing source/shadow pitches.
 *
 * IRQL: PASSIVE_LEVEL
 * ====================================================================== */
static NTSTATUS
DxgkpCopyPresentSourceToShadow(
    _In_ PDXGKRNL_ADAPTER        Adapter,
    _In_ PDXGKRNL_PRESENT_ENTRY  Entry)
{
    ...
    ULONG  DstPitch;
    ULONG  SrcPitch;
    ULONG  CopyBytesPerRow;
    ULONG  Row;
    PUCHAR Dst;
    PUCHAR Src;

    SourceAllocation =
        DxgkVidMmHandleToAllocation((HANDLE)(ULONG_PTR)Entry->hSource);
    ...
    /* Make sure a shadow framebuffer exists to copy into. */
    Status = DxgkpEnsureShadowFb(Adapter);
    if (!NT_SUCCESS(Status))
        return Status;
    ...
    DstPitch = Adapter->ShadowFbPitch ? Adapter->ShadowFbPitch : (Width * 4);
    ...
    _SEH2_TRY
    {
        for (Row = 0; Row < Height; Row++)
        {
            RtlCopyMemory(Dst + (SIZE_T)Row * DstPitch,
                          Src + (SIZE_T)Row * SrcPitch,
                          CopyBytesPerRow);
        }
    ...
    {
        Status = _SEH2_GetExceptionCode();
        DXGKRNL_WARN("DxgkpCopyPresentSourceToShadow: FAULTED 0x%08lX copying "
                     "hSource=0x%X src=%p dst=%p\n",
                     Status, Entry->hSource, SourceAddress, Adapter->ShadowFb);
    }
    _SEH2_END;
    ...
    _In_ PDXGKRNL_PRESENT_ENTRY  Entry)
{
    ...
    {
        DXGKRNL_TRACE("DxgkpExecuteDodPresent: VidPn not committed\n");
        return STATUS_DEVICE_NOT_READY;
    }

    /*
     * If a user present source allocation is supplied (e.g. dwm.exe's
     * composition allocation), copy its CPU-visible content into the
     * shadow framebuffer first.  The DOD miniport's DxgkDdiPresentDisplayOnly
     * blts the shadow framebuffer to the scanout, so this is how a kernel
     * D3DKMTPresent of a complete frame reaches the display tear-free.
     *
     * The shadow framebuffer is normally created by the GDI display path's
     * IOCTL_VIDEO_MAP_VIDEO_MEMORY.  When the GDI desktop lives on a
     * different display (e.g. the GOP framebuffer at Video0) it may not
     * exist yet, so allocate it on demand here.
     */
    if (Entry->hSource != 0)
    {
        NTSTATUS CopyStatus = DxgkpCopyPresentSourceToShadow(Adapter, Entry);
        if (!NT_SUCCESS(CopyStatus))
        {
            DXGKRNL_WARN("DxgkpExecuteDodPresent: source->shadow copy failed "
                         "0x%08lX (hSource=0x%X)\n",
                         CopyStatus, Entry->hSource);
            /* Continue: present whatever is currently in the shadow FB. */
        }
    }

    if (Adapter->ShadowFb == NULL)
    {
        DXGKRNL_TRACE("DxgkpExecuteDodPresent: no shadow FB\n");
        return STATUS_DEVICE_NOT_READY;
    }
