    ...
    _In_ ULONG64 PresentId)
{
    ...
    Status = DxgkpProgramScanoutViaMpo(Adapter, Allocation, VidPnSourceId,
                                       PrimaryAddress);
    {
        static LONG ScanoutDiag = 0;

        if (InterlockedIncrement(&ScanoutDiag) <= 8)
        {
            DbgPrint("DXGKRNL: scanout-program #%ld: MPO=0x%08lX "
                     "addr=%08lx%08lx seg=%u\n",
                     ScanoutDiag, Status,
                     (ULONG)(PrimaryAddress.QuadPart >> 32),
                     (ULONG)PrimaryAddress.QuadPart,
