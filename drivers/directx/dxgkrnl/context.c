    ...
    _Inout_ D3DKMT_CREATEDEVICE *pCreateDevice)
{
    ...
    /*
     * Display-Only Drivers (DOD) use the KMDDOD_INITIALIZATION_DATA layout
     * and have NO DxgkDdiCreateDevice DDI (they create no render devices).
     * Reading InitData.s.DxgkDdiCreateDevice on a DOD is meaningless (it
     * aliases an unrelated DOD field).  A dxgkrnl device is still needed so
     * user-mode present callers (e.g. dwm.exe) have a device handle to issue
     * D3DKMTPresent against — create one with no miniport device context.
     */
    if (Adapter->MiniportContext->IsDisplayOnlyDriver)
    {
        Device->hMiniportDevice = NULL;
        DXGKRNL_TRACE("DxgkCreateDevice: DOD adapter — device with no miniport "
                      "context\n");
    }
    else if (Adapter->MiniportContext->InitData.s.DxgkDdiCreateDevice != NULL)
    {
