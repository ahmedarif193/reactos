...
/* win32k registers dwm's vblank pacing event (input: ULONGLONG holding a
 * referenced PKEVENT; 0 unregisters). The present timer DPC — this display
 * path's scanout cadence — signals it every period, so dwm paces frames to
 * the real refresh instead of sleeping. The caller (win32k) owns the
 * reference and must unregister before releasing it.                  */
    ...
    _In_ PDXGKRNL_ADAPTER Adapter)
{
    if (Adapter->PresentTimerActive)
    {
         ...
         */
        KeFlushQueuedDpcs();
        while (InterlockedCompareExchange(&g_PresentDispatchBusy, 0, 0) != 0)
        {
            LARGE_INTEGER Delay;
