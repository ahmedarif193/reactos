...
/*
 * When the firmware handed off a UEFI GOP framebuffer, the GDI desktop is
 * driven by the uefifb XPDM miniport on \Device\Video0.  dxgkrnl must NOT
 * take Video0 in that case (win32k would pick it as the GDI primary and
 * bugcheck 0xB4 because a WDDM device has no XPDM DrvEnable) -- instead we
 * reserve Video0 for uefifb and register the WDDM display at Video1.
 */
BOOLEAN
NTAPI
    ...
    _In_ PDXGKRNL_ADAPTER Adapter)
{
    ...
    /*
     * Commit a real mode now even when a POST (GOP) framebuffer is present.
     * DxgkDisplayCommitVidPn's DOD path uses the POST display geometry and
     * sets VidPnCommitted + CommittedWidth/Height — which the DOD present
     * path (DxgkpExecuteDodPresent) requires.  Previously we returned early
     * whenever PostDisplayWidth/Height were set, leaving VidPnCommitted FALSE
     * so dwm's kernel D3DKMTPresent was rejected with DEVICE_NOT_READY.
     */
    ...
    _In_ PDXGKRNL_ADAPTER Adapter)
{
    ...
    else if (InbvHasValidGopFrameBuffer())
    {
        /*
         * A UEFI GOP framebuffer is present.  The uefifb XPDM miniport drives
         * the GDI desktop on \Device\Video0, but it is loaded by PnP/service
         * start AFTER this WDDM display registration runs.  Reserve Video0 for
         * it and place the WDDM display at Video1 so win32k keeps the GDI
         * desktop on the working GOP framebuffer.  dwm.exe still reaches this
         * adapter via D3DKMTOpenAdapterFromGdiDisplayName (which returns the
         * first started dxgkrnl adapter regardless of device number).
         */
        DeviceNumber = 1;
        DXGKRNL_TRACE("DxgkDisplayRegister: GOP framebuffer present, reserving "
                       "Video0 for uefifb, starting WDDM display at Video1\n");
    }
    else
    {
