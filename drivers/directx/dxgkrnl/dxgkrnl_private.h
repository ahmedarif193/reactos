...
/*
 * MmSystemRangeStart is an exported kernel global declared in the per-arch
 * <mm.h>.  On ARM64 it does not surface in this TU's include chain, so declare
 * it explicitly (compatible redeclaration on the arches where it already is).
 */
#if defined(_M_ARM64)
