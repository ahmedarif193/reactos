    ...
    _Out_ PVOID              *VirtualAddress)
{
    ...
    *VirtualAddress = Va;
     ...
     */
    {
        ...
        /* MmMapIoSpace already invalidates the TLB; a system barrier makes the
         * new mappings visible before the caller dereferences the VA. */
...
#if defined(_M_ARM64) || defined(_M_ARM)
    /*
     * The VBE DISPI path uses x86 I/O ports (0x01CE/0x01CF) which do not
     * exist on ARM — issuing READ_PORT/WRITE_PORT there would fault.  On ARM
     * the only display source is the firmware GOP framebuffer (handled by the
     * caller before this fallback), so there is nothing to do here.
     */
    ...
    _Out_ PVOID   DisplayInformationArg)
{
    ...
    /*
     * Translate the GOP framebuffer pixel description to a D3DDDIFMT value.
     *
     * IMPORTANT (ReactOS ARM64 convention): FreeLOADer stores the framebuffer
     * BITS-PER-PIXEL in LOADER_PARAMETER_FRAMEBUFFER.PixelFormat (see
     * winldr.c: GopFramebuffer.PixelFormat = FrameBufferData->BitsPerPixel),
     * NOT the EFI GOP EFI_GRAPHICS_PIXEL_FORMAT enum.  The actual colour
     * layout is carried by RedMask/GreenMask/BlueMask.  InbV's GOP code uses
     * the same convention.  So Fb.PixelFormat == 32 means 32bpp, == 16 means
     * 16bpp, etc.  Interpreting it as the EFI enum (0..3) is wrong and was
     * causing the display-only miniport to receive a zeroed framebuffer.
     */
    switch (Fb.PixelFormat)
    {
        case 32: /* 32bpp XRGB/XBGR — byte order is implied by the masks */
            ColorFormat   = D3DDDIFMT_X8R8G8B8;
            BytesPerPixel = 4;
            break;

        case 16: /* 16bpp — R5G6B5 if masks say so, else assume R5G6B5 */
            ColorFormat   = D3DDDIFMT_R5G6B5;
            BytesPerPixel = 2;
            break;

        case 24: /* 24bpp packed RGB */
            ColorFormat   = D3DDDIFMT_R8G8B8;
            BytesPerPixel = 3;
            break;

        case 15: /* 15bpp X1R5G5B5 */
            ColorFormat   = D3DDDIFMT_X1R5G5B5;
            BytesPerPixel = 2;
            break;

        default:
            /*
             * Unknown BPP.  If a 16bpp R5G6B5 mask is present treat as 16bpp,
             * otherwise fall back to 32bpp X8R8G8B8 (the common case) rather
             * than rejecting the framebuffer outright.
             */
            ...
            else
            {
                DXGKRNL_WARN("DxgkCbAcquirePostDisplayOwnership: "
                             "unexpected BPP %lu — assuming 32bpp X8R8G8B8\n",
                             Fb.PixelFormat);
                ColorFormat   = D3DDDIFMT_X8R8G8B8;
                BytesPerPixel = 4;
            }
    ...
    _In_ PDRIVER_INITIALIZATION_DATA DriverInitializationData)
{
    ...
    /* Debug: dump full raw init data to find viogpudo's callback layout. */
    if (MpCtx->InitData.s.Version >= 0x3000)
    {
        PULONG_PTR Raw = (PULONG_PTR)&MpCtx->InitData;
        ULONG Slots = CopySize / sizeof(ULONG_PTR);
        ULONG Idx;
        DXGKRNL_TRACE("DxgkInitializeEx: raw InitData[%lu slots]:\n", Slots);
        for (Idx = 0; Idx < Slots && Idx < 62; Idx += 4)
        {
            DXGKRNL_TRACE("  [%02u] %p %p %p %p\n", Idx,
                          (PVOID)Raw[Idx], (PVOID)(Idx+1 < Slots ? Raw[Idx+1] : 0),
                          (PVOID)(Idx+2 < Slots ? Raw[Idx+2] : 0),
                          (PVOID)(Idx+3 < Slots ? Raw[Idx+3] : 0));
        }
    }
