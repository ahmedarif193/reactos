    ...
    _In_ PDXGKRNL_ADAPTER Adapter)
{
    ...
    while (InterlockedCompareExchange(&Adapter->TdrWorkQueued, 0, 0) != 0)
    {
        LARGE_INTEGER Delay;
    ...
    _In_ UINT OpenHandleCount)
{
    ...
    Entry->NodeOrdinal = NodeOrdinal % DXGK_MAX_TRACKED_NODES;
    Entry->hSignalSyncObject = hSignalSyncObject;
    Entry->SignalFenceValue = SignalFenceValue;
    Entry->RefreshPresentId = PresentId;
    if (Device != NULL)
        InterlockedIncrement(&Device->InFlightSubmissions);
    Entry->Buffer = Buffer;
    Entry->Tag = Tag;
    Entry->Device = Device;
    ...
    _In_ CONST DXGKARGCB_NOTIFY_INTERRUPT_DATA    *NotifyInterruptData)
{
    ...
    if (NotifyInterruptData)
    {
        ...
        else if (NotifyInterruptData->InterruptType == DXGK_INTERRUPT_CRTC_VSYNC)
        {
            ...
            InterlockedIncrement(&Adapter->VsyncCount);
        }
        if (c <= 10)
        {
