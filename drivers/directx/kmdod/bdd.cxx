...
/*
 * VBE DISPI register access (bochs-display / QEMU stdvga, x86 only).
 * ARM has no port I/O space, so these are no-ops there; the GOP framebuffer
 * path (DxgkCbAcquirePostDisplayOwnership) is used on ARM instead.
 * Non-paged: also reachable from SystemDisplayEnable (bugcheck path).
 */
static USHORT
...
BOOLEAN BASIC_DISPLAY_DRIVER::TryVbeDisplayFallback()
{
     ...
     *
     * On ARM the VBE DISPI accessors are no-ops (no port I/O), so DispiId
     * reads back 0 and this returns FALSE — the GOP framebuffer path is
     * the only display source there.
     */
