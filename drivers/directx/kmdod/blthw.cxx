...
#define KMDOD_HW_TRACE_LOG_LIMIT  16
#define KMDOD_HW_TRACE_SLOW_US    5000ULL

static volatile LONG g_KmdodHwPresentTraceCount = 0;
static volatile LONG g_KmdodHwBltTraceCount = 0;

FORCEINLINE ULONGLONG
KmdodHwTraceNow100ns(VOID)
{
    return KeQueryInterruptTime();
}

FORCEINLINE ULONGLONG
KmdodHwTraceElapsedUs(
...
--*/
{
    ...
    ULONGLONG Start100ns = KmdodHwTraceNow100ns();
    ULONGLONG ElapsedUs;
    LONG TraceSeq;
...

    {
        ...
        // ReactOS/MinGW: __try/__except (PSEH2) does not work reliably in C++ on amd64.
        // Call MmProbeAndLockPages directly; if it fails it raises an exception
        // which will bugcheck -- acceptable for an initial port.
    ...
    if (m_SynchExecution)
    {
        ...
        ElapsedUs = KmdodHwTraceElapsedUs(Start100ns);
        TraceSeq = InterlockedIncrement(&g_KmdodHwPresentTraceCount);
        if (TraceSeq <= KMDOD_HW_TRACE_LOG_LIMIT ||
            ElapsedUs >= KMDOD_HW_TRACE_SLOW_US)
        {
            DbgPrint("KMDOD: ExecutePresentDisplayOnly seq=%ld sync=%u status=0x%08lx dur=%I64u us moves=%lu dirty=%lu src=%ux%u\n",
                     TraceSeq,
                     m_SynchExecution,
                     Status,
                     ElapsedUs,
    ...
    else
    {
        ...
        Status = StartHwBltPresentWorkerThread(HwContextWorkerThread,(PVOID)ctx);
        ElapsedUs = KmdodHwTraceElapsedUs(Start100ns);
        TraceSeq = InterlockedIncrement(&g_KmdodHwPresentTraceCount);
        if (TraceSeq <= KMDOD_HW_TRACE_LOG_LIMIT ||
            ElapsedUs >= KMDOD_HW_TRACE_SLOW_US)
...
--*/
{
    ...
    ElapsedUs = KmdodHwTraceElapsedUs(Start100ns);
    TraceSeq = InterlockedIncrement(&g_KmdodHwBltTraceCount);
    if (TraceSeq <= KMDOD_HW_TRACE_LOG_LIMIT ||
        ElapsedUs >= KMDOD_HW_TRACE_SLOW_US)
    {
        DbgPrint("KMDOD: HwExecutePresentDisplayOnly seq=%ld sync=%u dur=%I64u us moves=%lu dirty=%lu src=%ux%u pitch=%ld\n",
                 TraceSeq,
                 ctx->SynchExecution,
                 ElapsedUs,
                 ctx->NumMoves,
