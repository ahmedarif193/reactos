...
extern "C"
{
    ...
    // Windows DDI headers
    // winddi.h is not needed for KMDOD and causes dllimport issues on ReactOS
    // #include <winddi.h>
    #include <ntddvdeo.h>
...
typedef struct _BDD_FLAGS
{
    ...
    UINT IsInternal              : 1; // ( 6) 1 if internal panel detected (e.g. laptop)
    UINT VbeFallbackActive       : 1; // ( 7) 1 if VBE DISPI fallback was used to set display mode
...
private:
    ...
    NTSTATUS InitializeCurrentMode(D3DDDI_VIDEO_PRESENT_SOURCE_ID SourceId);

    NTSTATUS GetFrameBufferLength(D3DDDI_VIDEO_PRESENT_SOURCE_ID SourceId,
    ...
    // Try VBE DISPI register programming to establish a linear framebuffer
    BOOLEAN TryVbeDisplayFallback();
