...
/*
 * WDDM 1.0 (Vista SP1) interface version.
 * d3dukmdt.h defines DXGKDDI_INTERFACE_VERSION_VISTA      = 0x1052 (Vista RTM)
 *                 and DXGKDDI_INTERFACE_VERSION_VISTA_SP1  = 0x1053 (Vista SP1)
 * We target Vista SP1 which is the minimum for full WDDM 1.0 compliance.
 * The build flag passes -DDXGKDDI_INTERFACE_VERSION=0x1052 (Vista RTM) but
 * the version field in DRIVER_INITIALIZATION_DATA must be SP1 to enable the
 * full DDI table.
 */
#ifndef DXGKDDI_INTERFACE_VERSION_VISTA_SP1
    ...
    _In_ PUNICODE_STRING RegistryPath)
{
    ...
    /*
     * Version field must be set to DXGKDDI_INTERFACE_VERSION_VISTA_SP1 (0x1053)
     * so that dxgkrnl knows this is a WDDM 1.0 SP1 miniport.
     */
    InitData.Version = DXGKDDI_INTERFACE_VERSION_VISTA_SP1;
    ...
    case DXGKQAITYPE_QUERYSEGMENT:
    {
        ...
        if (pSegOut->pSegmentDescriptor == NULL)
        {
            ...
            DPRINT("SOFTGPU: QueryAdapterInfo QUERYSEGMENT phase 1: "
                   "reporting 1 segment\n");
            pSegOut->NbSegment            = 1;
            pSegOut->PagingBufferSegmentId = SOFTGPU_SEGMENT_ID;
        ...
        pDesc->BaseAddress.QuadPart          = Device->FrameBufferPhys.QuadPart;
        pDesc->CpuTranslatedAddress.QuadPart = Device->FrameBufferPhys.QuadPart;
        pDesc->Size                          = Device->FrameBufferSize;
        pDesc->CommitLimit                   = Device->FrameBufferSize;
    ...
    _In_  ULONG                  ChildRelationsSize)
{
     ...
     * HpdAwareness = 0 means the device cannot generate hot-plug events.
     */
    RtlZeroMemory(ChildRelations, sizeof(DXGK_CHILD_DESCRIPTOR));
    ChildRelations[0].ChildDeviceType                     = TypeVideoOutput;
    ChildRelations[0].ChildCapabilities.Type.HpdAwareness = 0;
