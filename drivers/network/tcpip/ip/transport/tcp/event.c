...
TCPConnectEventHandler(void *arg, const err_t err)
{
    ...
    while (!IsListEmpty(&Connection->ConnectRequest))
    {
            ...
            ReturnInfo->RemoteAddressLength >= sizeof(TA_IP_ADDRESS))
        {
            ...
            Bucket->Status = TCPTranslateError(LibTCPGetPeerName(Connection->SocketContext, &ipaddr, &RemoteAddress->Address[0].Address[0].sin_port));
            RemoteAddress->Address[0].Address[0].sin_port = WH2N(RemoteAddress->Address[0].Address[0].sin_port);
            RemoteAddress->Address[0].Address[0].in_addr = ipaddr.addr;
            RtlZeroMemory(&RemoteAddress->Address[0].Address[0].sin_zero, sizeof(RemoteAddress->Address[0].Address[0].sin_zero));
        }
