    ...
    IN PIRP Irp)
{
    ...
    {
        if (ParentId)
            ExFreePoolWithTag(ParentId, 0); // Allocated by parent driver, presumably PagedPool or generic P PNP pool? 
                                            // Actually PnP strings are usually NonPaged or Paged. 
                                            // IoQueryId rules say caller frees with ExFreePool.
                                            // We don't know the tag, 0 is safe usually or default tag.
