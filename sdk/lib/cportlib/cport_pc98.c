    ...
    IN ULONG   Length)
{
    ...
    if (Port->Address == Rs232ComPort[0].Address && Rs232ComPort[0].HasFifo)
    {
        /* SER1 with FIFO: batch up to 16 bytes per TxRDY wait */
        while (Length > 0)
        {
            ULONG Chunk = (Length < 16) ? Length : 16;

            while ((CpReadLsr(Port, SER1_LSR_TxRDY) & SER1_LSR_TxRDY) == 0)
                NOTHING;

            for (i = 0; i < Chunk; i++)
                WRITE_PORT_UCHAR((PUCHAR)SER1_IO_o_TRANSMITTER_BUFFER, *Buffer++);

            Length -= Chunk;
        }
    }
    else if (Port->Address == Rs232ComPort[1].Address && Rs232ComPort[1].HasFifo)
    {
        /* SER2 (ns16550-compatible) with FIFO: batch up to 16 bytes per THR empty wait */
        while (Length > 0)
        {
            ULONG Chunk = (Length < 16) ? Length : 16;
            ...
            for (i = 0; i < Chunk; i++)
                WRITE_PORT_UCHAR((PUCHAR)SER2_IO_o_TRANSMITTER_BUFFER, *Buffer++);
    ...
    {
        /* No FIFO (SER1 legacy or unknown): byte-by-byte fallback */
        for (i = 0; i < Length; i++)
