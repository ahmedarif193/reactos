...
void set_config_value(const char *kv)
{
    ...
    key = xstrndup(kv, (size_t)(eq - kv));
    o = find_opt(key);
    if (!o)
        die("--set names unknown option '%s'", key);
    value = xstrdup(eq + 1);
    rtrim(value);
    if (o->type == OPT_BOOL && !bool_value_ok(o, value))
        die("--set gives invalid value '%s' for %s", value, key);
    if (o->type == OPT_CHOICE && choice_index(o, value) < 0)
        die("--set gives unknown value '%s' for %s", value, key);
    set_value(o, value);
    free(value);
    free(key);
}
...
int cache_save(const char *path)
{
    ...
    fprintf(f, "# Generated by rosconfig " ROSCONFIG_VERSION "; edit from this output tree\n");
    fprintf(f, "# with menuconfig instead of by hand. Unknown lines are preserved.\n");
    for (i = 0; i < g_nopts; i++) {
        Option *o = &g_opts[i];
        if (o->menu != last_menu) {
            int chain[64];
...
int generate_cmake(const char *path)
{
    ...
    fprintf(f, "# Generated by rosconfig " ROSCONFIG_VERSION " from this output tree's .rosconfig/config.cache.\n");
    fprintf(f, "# Run menuconfig from the output tree (or use --build-dir)" " to change these values.\n");
