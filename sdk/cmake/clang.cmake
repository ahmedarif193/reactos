...
if(NOT CLANG_RESOURCE_DIR)
    execute_process(COMMAND ${CMAKE_C_COMPILER} -print-resource-dir OUTPUT_VARIABLE _clang_resource_dir OUTPUT_STRIP_TRAILING_WHITESPACE)
    set(CLANG_RESOURCE_DIR "${_clang_resource_dir}" CACHE INTERNAL "Clang resource directory")
endif()
...
# Locate a GNU binutils/gcc cross toolchain for the assembly LLVM's integrated
# assembler cannot handle (altmacro '&' concatenation, --32-style GNU as
# arguments). Default to the RosBE layout where a gcc-<cpu> toolchain sits
# next to the llvm-mingw install.
if(NOT REACTOS_CLANG_GCC_TOOLCHAIN)
    if(ARCH STREQUAL "i386")
        set(_rosbe_gnu_dirname gcc-i686)
    elseif(ARCH STREQUAL "amd64")
        set(_rosbe_gnu_dirname gcc-x86_64)
    else()
        set(_rosbe_gnu_dirname "")
    endif()
    if(_rosbe_gnu_dirname)
        get_filename_component(_clang_bindir "${CMAKE_C_COMPILER}" DIRECTORY)
        get_filename_component(_clang_root "${_clang_bindir}" DIRECTORY)
        get_filename_component(_toolchains_root "${_clang_root}" DIRECTORY)
        if(EXISTS "${_toolchains_root}/${_rosbe_gnu_dirname}/bin/${CMAKE_C_COMPILER_TARGET}-as")
            set(REACTOS_CLANG_GCC_TOOLCHAIN "${_toolchains_root}/${_rosbe_gnu_dirname}"
                CACHE PATH "GNU gcc cross toolchain used next to Clang")
            message(STATUS "Using GNU toolchain for assembly: ${REACTOS_CLANG_GCC_TOOLCHAIN}")
        endif()
    endif()
endif()

if(REACTOS_CLANG_GCC_TOOLCHAIN)
    # The flag set that routes a .S compile through the GNU assembler, keeping
    # asm.inc on its GAS path (-U__clang__). Applied globally on i386; on
    # amd64 only per-source where needed (ke/amd64/trap.S in ntos.cmake),
    # because GNU as encodes the 16-bit fragments of freeldr's mixed-mode .S
    # with COFF relocations lld does not implement.
    set(REACTOS_CLANG_GNU_AS_FLAGS
        "-fno-integrated-as;-U__clang__;--prefix=${REACTOS_CLANG_GCC_TOOLCHAIN}/bin/${CMAKE_C_COMPILER_TARGET}-")
endif()

if(ARCH STREQUAL "i386")
    add_compile_options(-fno-optimize-sibling-calls -fno-omit-frame-pointer -mstackrealign)
    if(REACTOS_CLANG_GNU_AS_FLAGS)
        # llvm-mingw's triplet "as" wrappers forward back into Clang and do not
        # accept GNU as arguments like --32. Route all i386 assembly through
        # RosBE's real GNU as, the legacy asm still depends on its semantics.
        add_compile_options("$<$<COMPILE_LANGUAGE:ASM>:${REACTOS_CLANG_GNU_AS_FLAGS}>")
    endif()
...
if(SEPARATE_DBG)
    ...
    set(CMAKE_CXX_CREATE_SHARED_LIBRARY
        ...
        "${CMAKE_STRIP} --only-keep-debug <TARGET> -o ${REACTOS_BINARY_DIR}/symbols/${SYMBOL_FILE}"
        ${strip_debug})
    # NOTE: the <CMAKE_SHARED_LIBRARY_*_C_FLAGS> placeholders are only expanded
    # for targets whose linker language is C. For RC-only modules they expand
    # to nothing, and unlike GNU ld, lld refuses a no-entry link that is not
    # marked -shared. Bake the C values in at configure time instead.
    ...
    # See the SEPARATE_DBG note above: bake the C shared-library flags in at
    # configure time, the <CMAKE_SHARED_LIBRARY_*_C_FLAGS> placeholders are not
    # expanded for RC-linked modules and lld requires -shared for -Wl,--entry=.
...
set(CMAKE_C_COMPILE_OBJECT "<CMAKE_C_COMPILER> <DEFINES> ${CLANG_RESOURCE_INCLUDE_FLAG} <INCLUDES> <FLAGS> -o <OBJECT> -c <SOURCE>")
...
function(set_subsystem MODULE SUBSYSTEM)
    # lld's MinGW driver only understands named subsystems
...
if(NOT REACTOS_CLANG_LLVM_MINGW_ROOT)
    # Default to the install the compiler itself lives in (e.g. an llvm-mingw
    # toolchain put on PATH by RosBE): <root>/bin/clang -> <root>
    get_filename_component(_clang_bindir "${CMAKE_C_COMPILER}" DIRECTORY)
    get_filename_component(_clang_root "${_clang_bindir}" DIRECTORY)
    if(EXISTS "${_clang_root}/${_gnu_tool_triplet}/lib/libc++.a")
        set(REACTOS_CLANG_LLVM_MINGW_ROOT "${_clang_root}" CACHE PATH "llvm-mingw toolchain root")
    endif()
endif()
...
# The runtime libraries live at fixed paths in an llvm-mingw install: the
# target sysroot for the C/C++ runtime and the compiler resource dir
# (already queried above) for the compiler-rt builtins. Resolving them
# directly avoids a batch of -print-file-name compiler launches on every
# configure.
set(_llvm_mingw_libdir "${REACTOS_CLANG_LLVM_MINGW_ROOT}/${_gnu_tool_triplet}/lib")
set(_clang_rt_builtins "${CLANG_RESOURCE_DIR}/lib/windows/${_clang_builtins_name}")
set(_llvm_libcxx "${_llvm_mingw_libdir}/libc++.a")
set(_llvm_libcxxabi "${_llvm_mingw_libdir}/libc++abi.a")
set(_llvm_libunwind "${_llvm_mingw_libdir}/libunwind.a")
set(_mingwex_lib "${_llvm_mingw_libdir}/libmingwex.a")

set(_missing_target_runtime)
foreach(_runtime_lib _clang_rt_builtins _llvm_libcxx _llvm_libcxxabi _llvm_libunwind _mingwex_lib)
    if(NOT EXISTS "${${_runtime_lib}}")
        list(APPEND _missing_target_runtime "${${_runtime_lib}}")
    endif()
endforeach()
...
# The builtins archive is linked into everything right here; libgcc is
# only the interface point for modules that name it explicitly
add_library(libgcc INTERFACE)
target_link_libraries(libgcc INTERFACE libkernel32)
link_libraries(${_clang_rt_builtins})
# toolchain-clang.cmake seeds -lgcc for the legacy GNU runtime; with the
# llvm-mingw runtime there is no libgcc.a, compiler-rt builtins are linked
# explicitly above instead.
set(CMAKE_C_STANDARD_LIBRARIES "")
set(CMAKE_CXX_STANDARD_LIBRARIES "")
# A few modules (hal, vfatxlib, rostests) still pass a literal -lgcc:
# satisfy it with an empty archive (the builtins already come from the
# global link_libraries above). Remove any previous shim first - writing
# through a leftover symlink would clobber its target.
set(_libgcc_shim_dir ${REACTOS_BINARY_DIR}/sdk/lib/clang-libgcc)
file(MAKE_DIRECTORY ${_libgcc_shim_dir})
file(REMOVE ${_libgcc_shim_dir}/libgcc.a)
file(WRITE ${_libgcc_shim_dir}/libgcc.a "!<arch>\n")
link_directories(${_libgcc_shim_dir})
...
add_compile_options(
    ...
    foreach(_include_dir
            "${REACTOS_CLANG_LLVM_MINGW_ROOT}/${_gnu_tool_triplet}/include/c++/v1"
            "${REACTOS_CLANG_LLVM_MINGW_ROOT}/include/c++/v1")
