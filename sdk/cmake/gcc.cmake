...
if(CMAKE_C_COMPILER_ID STREQUAL "Clang" AND _LIBSTDCCXX_INCLUDE_DIRS)
    foreach(_STDCPP_INCLUDE_DIR IN LISTS _LIBSTDCCXX_INCLUDE_DIRS)
        add_compile_options(-isystem${_STDCPP_INCLUDE_DIR})
    endforeach()
endif()
unset(_GXX_BIN_DIR)
...
if(_LIBSTDCCXX_INCLUDE_DIRS)
    set_property(TARGET cppstl PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${_LIBSTDCCXX_INCLUDE_DIRS}")
endif()
target_compile_definitions(cppstl INTERFACE "$<$<COMPILE_LANGUAGE:CXX>:PAL_STDCPP_COMPAT>")
unset(_LIBSTDCCXX_INCLUDE_DIRS)

if(CMAKE_C_COMPILER_ID STREQUAL "Clang")
    set(_REACTOS_CLANG_LINK_DIRS)
    foreach(_reactos_clang_lib_location
            ${LIBSTDCCXX_LOCATION}
            ${LIBMINGWEX_LOCATION}
            ${LIBSUPCXX_LOCATION}
            ${LIBGCC_LOCATION}
            ${LIBGCCEH_LOCATION})
        if(_reactos_clang_lib_location)
            get_filename_component(_reactos_clang_lib_dir "${_reactos_clang_lib_location}" DIRECTORY)
            if(EXISTS "${_reactos_clang_lib_dir}")
                list(APPEND _REACTOS_CLANG_LINK_DIRS "${_reactos_clang_lib_dir}")
            endif()
        endif()
    endforeach()
    list(REMOVE_DUPLICATES _REACTOS_CLANG_LINK_DIRS)
    if(_REACTOS_CLANG_LINK_DIRS)
        link_directories(${_REACTOS_CLANG_LINK_DIRS})
    endif()
    unset(_REACTOS_CLANG_LINK_DIRS)
endif()
