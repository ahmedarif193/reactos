...
function(ros_add_rust_executable _target)
    ...
    # On i686-pc-windows-gnu, the ReactOS build output dirs contain actual .dll
    # files (e.g. kernel32.dll). GNU ld's -l search falls back to linking against
    # the .dll directly when no import library (.a) is found, which produces
    # incorrectly decorated stdcall import symbols. Prepend the system MinGW
    # import library directory so -lkernel32 etc. find the proper .a first.
    if(_triple STREQUAL "i686-pc-windows-gnu")
        execute_process(COMMAND ${ROS_RUST_LINKER} -print-file-name=libkernel32.a
            OUTPUT_VARIABLE _sys_kernel32_path OUTPUT_STRIP_TRAILING_WHITESPACE)
        if(EXISTS "${_sys_kernel32_path}")
            get_filename_component(_sys_importlib_dir "${_sys_kernel32_path}" DIRECTORY)
            set(_rust_native_flags "-L native=${_sys_importlib_dir}")
        endif()
    endif()
    ...
    # Also use ld.lld to avoid Apple ld on macOS which doesn't understand GNU flags.
    if(ROS_RUST_LINKER STREQUAL "${CMAKE_C_COMPILER}" AND CMAKE_C_COMPILER_ID STREQUAL "Clang")
        ...
        if(_clang_triplet)
            ...
            # Use LLD for cross-compilation (Apple ld doesn't understand GNU linker flags)
            set(_rust_target_extra "${_rust_target_extra} -C link-arg=-fuse-ld=lld")
        endif()
    endif()
    if(_triple STREQUAL "i686-pc-windows-gnu")
