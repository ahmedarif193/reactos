...
if(ROS_RUST_FOUND)
    ...
    if(NOT _ros_rustc_ver_rv EQUAL 0 OR NOT _ros_cargo_ver_rv EQUAL 0)
        message(FATAL_ERROR "Rust toolchain not usable. Configure a default toolchain (e.g., 'rustup default stable') or ensure rustc/cargo are installed.\n rustc: ${_ros_rustc_err}\n cargo: ${_ros_cargo_err}")
    endif()
endif()
...
if(ROS_RUST_FOUND AND ROS_RUSTUP_EXECUTABLE AND ROS_RUST_TARGET_TRIPLE)
    ...
    if(_ros_rustup_list_rv EQUAL 0)
        ...
        # rustup present but not configured; fail early so the problem is obvious
        set(ROS_RUST_TARGET_INSTALLED UNKNOWN)
        message(FATAL_ERROR "rustup is present but no default toolchain is configured. Run 'rustup default stable' or set RUSTUP_TOOLCHAIN, then re-run CMake.")
    endif()
endif()
...
if(CMAKE_C_COMPILER_ID STREQUAL "Clang")
    ...
    if(_mingw_triplet_for_link)
        set(ROS_RUST_LINKER "${_mingw_triplet_for_link}-gcc")
        set(ROS_RUST_AR "${_mingw_triplet_for_link}-ar")
    endif()
endif()
...
function(ros_add_rust_executable _target)
    ...
    if(NOT ROS_RUST_ENABLED)
        message(FATAL_ERROR "${_target}: Rust (cargo/rustc) not found. Install Rust and ensure ENABLE_RUST is ON.")
    endif()
...
if(ROS_RUST_FOUND)
    ...
    message(FATAL_ERROR "Rust not found (cargo/rustc missing). Install Rust and reconfigure, or set ENABLE_RUST=OFF.")
endif()
