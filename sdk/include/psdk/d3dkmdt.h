...
/*
 * POWER_ACTION and DEVICE_POWER_STATE are already provided by winnt.h on
 * MinGW and by the ReactOS DDK.  Only define them when neither has been
 * included yet (kernel-only build paths).
 */
#if !defined(_WINNT_) && !defined(__WINE_WINNT_H) && !defined(_NTDEF_)
typedef enum {
    PowerActionNone = 0,
    PowerActionReserved,
    PowerActionSleep,
...
typedef enum _D3DKMDT_VIDEO_OUTPUT_TECHNOLOGY
{
    ...
    D3DKMDT_VOT_INTERNAL                 = (-2147483647 - 1),
...
#endif /* _DXGK_NODEMETADATA_DEFINED */
...
typedef struct _D3DKMT_WDDM_3_1_CAPS
{
    union
    {
        struct
        {
            UINT    NativeGpuFenceSupported     :  1;   // Specifies whether the GPU supports native fence
            UINT    Reserved                    : 31;
        };
        UINT Value;
    };
...
C_ASSERT(sizeof(D3DKMT_WDDM_3_1_CAPS) == sizeof(UINT));
