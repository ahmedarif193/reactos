...
/* Bring-up fail-stop, not an NT fast-fail exception-delivery ABI. Preserve the
 * reason in a0 for trap diagnostics and never resume into the caller. */
static __inline__ __attribute__((__always_inline__, __noreturn__)) void __riscv_fastfail(unsigned int Code)
{
    ...
    __asm__ __volatile__("1: unimp\n\tj 1b" :: "r"(Reason) : "memory");
