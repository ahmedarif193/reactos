...
typedef enum _KI_RISCV_STARTUP_FAILURE
{
    ...
    KiRiscvStartupInvalidDirectMap,
    ...
    KiRiscvStartupPageTableAccessFailure,
...
static
BOOLEAN
KiRiscvAddressInFixedDirectMap(
    ...
    _In_ SIZE_T Size)
{
    ...
    if ((Size == 0) || !MiRiscvIsCanonicalAddress(Address) ||
        (Address < RISCV64_LOADER_DIRECT_MAP_BASE))
    {
        return FALSE;
    }

    Offset = Address - RISCV64_LOADER_DIRECT_MAP_BASE;
    return (Offset < RISCV64_LOADER_DIRECT_MAP_SIZE) &&
           (Size <= RISCV64_LOADER_DIRECT_MAP_SIZE - Offset);
}

static
PMMPTE
NTAPI
KiRiscvMapBootTablePage(
    _In_ PFN_NUMBER PageFrameNumber,
    _In_opt_ PVOID Context)
{
    PRISCV64_LOADER_BLOCK RiscvBlock = Context;
    ULONG64 PhysicalAddress;

    if ((RiscvBlock == NULL) ||
        (PageFrameNumber >
         (RiscvBlock->HighestMappedPhysicalAddress >> PAGE_SHIFT)))
    {
        return NULL;
    }

    PhysicalAddress = (ULONG64)PageFrameNumber << PAGE_SHIFT;
    if (PhysicalAddress > RiscvBlock->DirectMapSize - PAGE_SIZE)
        return NULL;

    return (PMMPTE)(ULONG_PTR)(RiscvBlock->DirectMapBase + PhysicalAddress);
}
    ...
    _In_ SIZE_T Size)
{
    ...
    PFN_NUMBER RootPageFrame, MaximumPageFrame;
    MI_RISCV_PAGE_WALK Walk;
    NTSTATUS Status;

    if (!KiRiscvAddressInFixedDirectMap(Address, Size))
        return FALSE;

    Last = Address + Size - 1;
    if ((Address - RiscvBlock->DirectMapBase >
         RiscvBlock->HighestMappedPhysicalAddress) ||
        (Last - RiscvBlock->DirectMapBase >
         RiscvBlock->HighestMappedPhysicalAddress))
    {
        return FALSE;
    }

    RootPageFrame = RiscvBlock->PageTableRoot >> PAGE_SHIFT;
    MaximumPageFrame = RiscvBlock->HighestMappedPhysicalAddress >> PAGE_SHIFT;
    Current = ALIGN_DOWN_BY(Address, PAGE_SIZE);
    for (;;)
    {
        Status = MiRiscvWalkPageTables(RootPageFrame,
                                       Current,
                                       MaximumPageFrame,
                                       KiRiscvMapBootTablePage,
                                       RiscvBlock,
        ...
        if (!NT_SUCCESS(Status) ||
            (Walk.PhysicalAddress.QuadPart !=
             Current - RiscvBlock->DirectMapBase) ||
            !Walk.Value.u.Hard.Read || !Walk.Value.u.Hard.Write ||
            !Walk.Value.u.Hard.Execute || Walk.Value.u.Hard.Owner ||
            !Walk.Global)
        {
            return FALSE;
        }
    ...
    _Inout_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    if (!KiRiscvAddressInFixedDirectMap((ULONG_PTR)LoaderBlock,
                                        sizeof(*LoaderBlock)))
    {
    ...
    if ((RiscvBlock->DirectMapBase != RISCV64_LOADER_DIRECT_MAP_BASE) ||
        (RiscvBlock->DirectMapSize != RISCV64_LOADER_DIRECT_MAP_SIZE) ||
        (RiscvBlock->HighestMappedPhysicalAddress >=
         RiscvBlock->DirectMapSize))
    {
        return KiRiscvStartupInvalidDirectMap;
    }
    ...
    if ((RiscvBlock->DpcStack & 15) ||
        (RiscvBlock->DpcStack <
         RiscvBlock->DirectMapBase + KERNEL_STACK_SIZE) ||
        !KiRiscvValidateMappedRange(RiscvBlock,
    ...
    if (LoaderBlock->KernelStackSize != KERNEL_STACK_SIZE)
        return KiRiscvStartupInvalidStackMapping;
#endif
    if ((RiscvBlock->SharedUserData & (PAGE_SIZE - 1)) ||
        !KiRiscvValidateMappedRange(RiscvBlock,
                                    RiscvBlock->SharedUserData,
                                    PAGE_SIZE))
    {
    ...
    _Inout_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    Status = MiRiscvInitializePageTableAccess(
        KiRiscvMapBootTablePage,
        RiscvBlock,
        RiscvBlock->HighestMappedPhysicalAddress >> PAGE_SHIFT);
    if (!NT_SUCCESS(Status))
        KiRiscvStartupStop(KiRiscvStartupPageTableAccessFailure);
    ...
    SharedUserData = (PKUSER_SHARED_DATA)(ULONG_PTR)RiscvBlock->SharedUserData;
    MmWriteableSharedUserData = SharedUserData;
    KeMemoryBarrier();
