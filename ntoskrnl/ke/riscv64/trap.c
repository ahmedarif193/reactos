    ...
    _In_ PVOID Address)
{
    ...
    if (KeLoaderBlock == NULL)
        return;
    if ((Va >= RISCV64_LOADER_DIRECT_MAP_BASE) && (Va < RISCV64_LOADER_DIRECT_MAP_BASE + RISCV64_LOADER_DIRECT_MAP_SIZE))
    {
        PFN_NUMBER Page = (Va - RISCV64_LOADER_DIRECT_MAP_BASE) >> PAGE_SHIFT;
             ...
             Entry = Entry->Flink)
        {
            ...
            {
                DbgPrint("address %p is direct-map page %lx: loader type %lu (base %lx, %lx pages)\n",
                         Address, Page, Md->MemoryType, Md->BasePage, Md->PageCount);
                return;
            }
        }
        DbgPrint("address %p is direct-map page %lx: no loader descriptor\n", Address, Page);
    }
}
...
KiRiscvTrapHandler(_Inout_ PKTRAP_FRAME TrapFrame)
{
    ...
    if (!(Scause & (1ULL << 63)) && ((Code == 12) || (Code == 13) || (Code == 15)))
    {
        ...
        if (NT_SUCCESS(MiRiscvWalkCurrentPageTables(Address, &Walk)))
        {
            ...
            /* Software A/D management (no Svadu reliance, ABI-132): the leaf
             * already grants this access, only A (or D for a store) is clear.
             * Set them in the real entry and retry; NT would never see this
             * fault on hardware that updates the bits itself. */
            if (KiRiscvLeafPermitsAccess(Walk.Value.u.Long, Code, Mode, TrapFrame->Sstatus))
            {
                ...
                {
                    __atomic_compare_exchange_n(&Walk.Entry->u.Long, &Expected, Expected | Needed, FALSE, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
                    KeInvalidateTlbEntry(Address);
                    Thread->TrapFrame = TrapFrame->PreviousTrapFrame;
                    return;
                }

                /* The leaf permits the access and A/D are set: the translation
                 * was stale. Retry, but never loop on a fault this model
                 * cannot explain: hand repeats to MM and its bugcheck path. */
        ...
        /* A missing direct-map page is a loader/PFN adoption bug, not demand
         * paging: attribute it before MM raises PAGE_FAULT_IN_NONPAGED_AREA. */
        if (((ULONG_PTR)Address >= RISCV64_LOADER_DIRECT_MAP_BASE) &&
            ((ULONG_PTR)Address < RISCV64_LOADER_DIRECT_MAP_BASE + RISCV64_LOADER_DIRECT_MAP_SIZE))
        {
            DbgPrint("\n*** Direct-map fault at %p from %p: cause %lu, code %lx, leaf %llx\n", Address,
