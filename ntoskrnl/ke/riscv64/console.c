    ...
    _In_ PRISCV64_LOADER_BLOCK RiscvBlock)
{
    ...
    if ((Length < Window) ||
        (Address >= RiscvBlock->DirectMapSize) ||
        (Length > RiscvBlock->DirectMapSize - Address))
    {
        return FALSE;
    }
    ...
    _In_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
     ...
     * which exists only if this image lives inside the direct map. */
    if (!KiRiscvConsoleProbeSbi())
        return;
    KiRiscvConsoleSbiReadPhysical = 0;
    if ((BufferVa >= RiscvBlock->DirectMapBase) &&
        (BufferVa - RiscvBlock->DirectMapBase < RiscvBlock->DirectMapSize))
    {
        KiRiscvConsoleSbiReadPhysical = BufferVa - (ULONG_PTR)RiscvBlock->DirectMapBase;
    }
