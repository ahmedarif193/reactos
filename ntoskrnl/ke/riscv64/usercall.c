...
KiRiscvCheckUserPage(_In_ PVOID Address, _In_ ULONG Access, _Inout_ PKTRAP_FRAME Frame)
{
    ...
    /* A stack guard fault is a successful first half of stack growth: ARM3
     * consumes the old guard, installs the next one and leaves this page as
     * demand-zero. Complete that second fault before checking the leaf. */
...
NTSTATUS
NTAPI
KiRiscvContinue(_In_ PCONTEXT Context, _In_ BOOLEAN TestAlert)
{
    CONTEXT Captured;
    PKTHREAD Thread = KeGetCurrentThread();
    ...
    {
        Status = KiRiscvCopyFromUser(&Captured, Context, sizeof(Captured));
        if (!NT_SUCCESS(Status)) return Status;
    }
    else
        Captured = *Context;

    if (!(Captured.ContextFlags & CONTEXT_RISCV64) || (Captured.ContextFlags & ~CONTEXT_ALL))
        return STATUS_INVALID_PARAMETER;
    if ((Captured.ContextFlags & (CONTEXT_CONTROL & ~CONTEXT_RISCV64)) &&
        (!KiRiscvUserControlAddress(Captured.Pc) || (Captured.Pc & 1) ||
         !KiRiscvUserControlAddress(Captured.Sp) || (Captured.Sp & 15)))
        return STATUS_INVALID_PARAMETER;

    KeContextToTrapFrame(&Captured, NULL, Frame, Captured.ContextFlags, UserMode);
    if (TestAlert) KeTestAlertThread(UserMode);
