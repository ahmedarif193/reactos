...
static volatile LONG KiKprofTraceActiveSampleSessions;
                             ...
                             IN KPROFILE_SOURCE Source)
{
    ...
    if (ReadNoFence(&KiKprofTraceActiveSampleSessions))
...
C_ASSERT(sizeof(KPROF_TRACE_RECORD) == 144);
 ...
 *
 * Readers publish per-processor activity before consulting the table.  Image
 * unload removes the entry first, then waits for those readers to drain before
 * the loader unmaps the image and its .pdata section.
 */
...
KiKprofUnwindRemoveImage(
    IN PVOID ImageBase)
{
    ULONG Index, Position, Processor;
    KIRQL OldIrql;
    ...
    /* A later reader cannot select the removed image.  Wait until every
     * reader which could still hold its function table has left the unwind
     * path before KprofTraceImageEvent returns to the image unloader. */
    for (Processor = 0; Processor < MAXIMUM_PROCESSORS; Processor++)
    {
        while (InterlockedCompareExchange(&KiKprofChainActive[Processor], 0, 0) != 0)
            YieldProcessor();
    }
}
...
KiKprofUnwindSeedSystemImages(VOID)
{
    ...
    if (ReadNoFence(&KiKprofUnwindSeeded)) return;

    /* The module list remains linked until after an image is unmapped.  Hold
     * the system-load mutant through directory parsing so unload cannot
     * remove an unwind entry and then race a stale seed insertion. */
    ...
    KeWaitForSingleObject(&MmSystemLoadLock,
                          ...
                          FALSE,
                          NULL);
    if (ReadNoFence(&KiKprofUnwindSeeded)) goto Exit;

    /* The spin lock protects list traversal; the system-load mutant keeps
     * each snapshotted image mapped while its exception directory is read. */
    ...
    InterlockedExchange(&KiKprofUnwindSeeded, 1);
    ...
    OUT PULONG ChainFlags)
{
    ...
    *ChainFlags = 0;
    ...
    if (InterlockedCompareExchange(&KiKprofChainActive[Processor], 1, 0) != 0)
    {
        *ChainFlags |= KPROF_TRACE_CHAIN_STOPPED;
        return Depth;
    }
    ...
    OUT PKPROF_TRACE_SESSION *Sessions)
{
         ...
         NextEntry = NextEntry->Flink)
    {
        ...
        if (RequiredFlag && !(Session->Config.Flags & RequiredFlag)) continue;
        if (RequiredFlag &&
            !(Session->Config.Affinity & AFFINITY_MASK(Processor)))
        {
    ...
    IN OPTIONAL PETHREAD Thread)
{
    ...
    Record->Header.ProcessKey = KiKprofTraceProcessKey(Process);
    Record->Header.ThreadKey = KiKprofTraceThreadKey(Thread);
}
    ...
    *SessionOut = NULL;
    ...
    if (((Config->Flags & KPROF_TRACE_FLAG_CALLCHAIN) &&
         ((Config->MaximumStackDepth <= 1) ||
          (Config->MaximumStackDepth > KPROF_TRACE_MAX_FRAMES))) ||
    ...
    IN OUT PKPROF_TRACE_SESSION Session)
{
    ...
    if (Session->Config.Flags & KPROF_TRACE_FLAG_SAMPLE)
    {
        StartSource = KiAcquireProfileSourceLocked(Session->Source, NewSource);
        Session->SourceAcquired = TRUE;
    }
    /* Process exit takes the same list lock before publishing its terminal
       state.  Checking here closes the CONFIGURE-to-START insertion gap. */
    if (Session->TargetProcess &&
        PsGetProcessExitProcessCalled(Session->TargetProcess))
    {
        InterlockedExchange(&Session->TargetExited, 1);
    }
    ...
    if (Session->Config.Flags & KPROF_TRACE_FLAG_SAMPLE)
        InterlockedIncrement(&KiKprofTraceActiveSampleSessions);
...
Exit:
    ...
    if (NewSource && !StartSource) ExFreePoolWithTag(NewSource, 'forP');
    return Status;
}
    ...
    IN OUT PKPROF_TRACE_SESSION Session)
{
    ...
    InitializeListHead(&Session->ListEntry);
    if (Session->Config.Flags & KPROF_TRACE_FLAG_SAMPLE)
        InterlockedDecrement(&KiKprofTraceActiveSampleSessions);
    if (Session->Config.Flags & KPROF_TRACE_FLAG_CONTEXT_SWITCH)
    ...
    OUT PKPROF_TRACE_STATS Stats)
{
    ...
    Stats->TargetExited =
        InterlockedCompareExchange(&Session->TargetExited, 0, 0) != 0;
    for (Processor = 0; Processor < MAXIMUM_PROCESSORS; Processor++)
    {
        PKPROF_TRACE_RING Ring;
        KIRQL OldIrql = PASSIVE_LEVEL;
    ...
    IN KPROFILE_SOURCE Source)
{
    ...
    ULONG Count, Index, Processor, RecordFlags, CaptureDepth = 0;
    ...
    for (Index = 0; Index < Count; Index++)
    {
        ...
        CaptureDepth = max(CaptureDepth, Sessions[Index]->Config.MaximumStackDepth);
    }

#if defined(_M_AMD64)
    if (CaptureDepth > 1)
    {
        FrameCount = KiKprofTraceCaptureKernelChain(TrapFrame, Frames, CaptureDepth, &ChainFlags);
    }
#else
    UNREFERENCED_PARAMETER(CaptureDepth);
    ...
    for (Index = 0; Index < Count; Index++)
    {
        ...
        {
            ULONG SessionFrameCount = 0;
            ULONG SessionChainFlags = 0;

            if (Sessions[Index]->Config.Flags & KPROF_TRACE_FLAG_CALLCHAIN)
            {
                SessionFrameCount = min(FrameCount, Sessions[Index]->Config.MaximumStackDepth);
                SessionChainFlags = ChainFlags;
                if (FrameCount > SessionFrameCount)
                    SessionChainFlags |= KPROF_TRACE_CHAIN_TRUNCATED;
            }
            Record->Data.Sample.FrameCount = SessionFrameCount;
            Record->Data.Sample.ChainFlags = SessionChainFlags;
            Record->Header.Size = sizeof(KPROF_TRACE_RECORD);
            Record->Header.Flags = RecordFlags;
            if (SessionChainFlags &
                (KPROF_TRACE_CHAIN_TRUNCATED | KPROF_TRACE_CHAIN_STOPPED))
            {
                Record->Header.Flags |= KPROF_TRACE_RECORD_FLAG_TRUNCATED;
            }
            if (SessionFrameCount > 1)
            {
                Record->Header.Size = (USHORT)(sizeof(KPROF_TRACE_RECORD) + SessionFrameCount * sizeof(ULONGLONG));
            }
    ...
    IN BOOLEAN Create)
{
    ...
    Processor = KeGetCurrentProcessorNumber();
    Count = KiKprofTraceSnapshotSessions(0, Process, NULL, Sessions);
    if (!Count) return;
    ...
    for (Index = 0; Index < Count; Index++)
    {
        if (Sessions[Index]->Config.Flags & KPROF_TRACE_FLAG_PROCESS)
            KiKprofTraceWriteRecord(Sessions[Index], Processor, &Record);
        if (!Create && (Sessions[Index]->TargetProcess == Process))
        {
            InterlockedExchange(&Sessions[Index]->TargetExited, 1);
            KeSetEvent(&Sessions[Index]->DataEvent, IO_NO_INCREMENT, FALSE);
        }
    }
    KiKprofTraceReleaseSessions(Sessions, Count);
}
    ...
    IN ULONG Reason)
{
    ...
    Record.Data.Scheduler.OldProcessKey = KiKprofTraceProcessKey(OldProcess);
    Record.Data.Scheduler.OldThreadKey = KiKprofTraceThreadKey(OldExecutiveThread);
    Record.Data.Scheduler.NewProcessKey = KiKprofTraceProcessKey(NewProcess);
    Record.Data.Scheduler.NewThreadKey = KiKprofTraceThreadKey(NewExecutiveThread);
    ...
    IN ULONG TargetProcessor)
{
    ...
    Record.Data.Scheduler.NewProcessKey = KiKprofTraceProcessKey(Process);
    Record.Data.Scheduler.NewThreadKey = KiKprofTraceThreadKey(ExecutiveThread);
