 ...
 *
 * POLICY (see irql.c KiUpdateDaifForIrql):
 *   - SError is unmasked only at PASSIVE_LEVEL
 *   - Kernel-mode SError -> BUGCHECK (unrecoverable hardware error)
 *   - User-mode SError -> Terminate process (bad hardware access)
 */
BOOLEAN
NTAPI
KiSErrorHandler(
