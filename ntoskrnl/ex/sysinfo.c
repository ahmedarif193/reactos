        ...
        *ReqSize = sizeof(SYSTEM_PROCESS_INFORMATION);
        ...
        do
        {
            ...
            if (!Overflow)
            {
                ...
                SpiCurrent->PeakWorkingSetSize = Process->Vm.Instance.PeakWorkingSetSize << PAGE_SHIFT;
                SpiCurrent->WorkingSetSize = Process->Vm.Instance.WorkingSetSize << PAGE_SHIFT;
                SpiCurrent->WorkingSetPrivateSize.QuadPart =
                    (LONGLONG)(min(Process->NumberOfPrivatePages, Process->Vm.Instance.WorkingSetSize) << PAGE_SHIFT);
...
{
    PSYSTEM_PHYSICAL_MEMORY_INFORMATION MemInfo = Buffer;

    *ReqSize = sizeof(*MemInfo);
    ...
    MemInfo->TotalPhysicalBytes =
        (ULONGLONG)MmNumberOfPhysicalPages << PAGE_SHIFT;
    MemInfo->LowestPhysicalAddress =
        (ULONGLONG)MmLowestPhysicalPage << PAGE_SHIFT;
    MemInfo->HighestPhysicalAddress =
    ...
    _Out_opt_ PULONG ReturnLength)
{
    ...
    _SEH2_TRY
    {
        ...
        switch (SystemInformationClass)
        {
            ...
            {
                SYSTEM_SUPPORTED_PROCESSOR_ARCHITECTURES_INFORMATION *Machines = SystemInformation;
                ...
                ProcessHandle = *(volatile HANDLE *)InputBuffer;
                CapturedResultLength = MachineCount * sizeof(*Machines);
                if (ReturnLength) *ReturnLength = CapturedResultLength;
                if (SystemInformationLength < CapturedResultLength)
                {
                    Status = STATUS_BUFFER_TOO_SMALL;
                    break;
                }
                if (!Machines) _SEH2_YIELD(return STATUS_ACCESS_VIOLATION);
                ...
                RtlZeroMemory(Machines, CapturedResultLength);
                Machines[0].Machine = IMAGE_FILE_MACHINE_NATIVE;
                Machines[0].KernelMode = TRUE;
