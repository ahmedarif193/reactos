...
)
{
    ...
    if (FlagOn(Process->JobStatus, PSP_JOB_NOT_REALLY_ACTIVE) ||
        FlagOn(Process->Flags, PSF_PROCESS_DELETE_BIT))
    {
        goto Exit;
    }
...
)
{
    ...
    if ((ExtendedLimit->BasicLimitInformation.LimitFlags & JOB_OBJECT_LIMIT_SUBSET_AFFINITY) &&
        !(ExtendedLimit->BasicLimitInformation.LimitFlags & JOB_OBJECT_LIMIT_AFFINITY))
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (ExtendedLimit->BasicLimitInformation.LimitFlags & JOB_OBJECT_LIMIT_AFFINITY)
    {
        ...
        if (!ExtendedLimit->BasicLimitInformation.Affinity ||
            ExtendedLimit->BasicLimitInformation.Affinity !=
...
)
{
    ...
    case JobObjectBasicUIRestrictions:
    {
        ...
        _SEH2_END;
        if (UiRestrictions.UIRestrictionsClass & ~0xFF)
        {
            Status = STATUS_INVALID_PARAMETER;
            break;
        }
