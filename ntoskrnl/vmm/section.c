...
MmDisableModifiedWriteOfSection(IN PSECTION_OBJECT_POINTERS SectionObjectPointer)
{
     ...
     * never an ARM3 CONTROL_AREA in ReactOS, because ARM3 (MmCreateVmmSection)
...
MmUnmapViewInSystemSpace(IN PVOID MappedBase)
{
    ...
    if ((MemoryArea) && (MemoryArea->Type != MEMORY_AREA_OWNED_BY_VMM))
    {
...
VOID
NTAPI
MiDeleteVmmSection(PVOID ObjectBody)
{
    PSECTION SectionObject;
    PCONTROL_AREA ControlArea;
    KIRQL OldIrql;
