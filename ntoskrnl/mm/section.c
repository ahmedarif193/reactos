...
#define MI_MAX_FLUSH_RUN_PAGES (MM_MAXIMUM_DISK_IO_SIZE / PAGE_SIZE)
    ...
    _In_ ULONG PageCount)
{
    ...
    ASSERT(PageCount != 0);
    ASSERT(PageCount <= MI_MAX_FLUSH_RUN_PAGES);
    ...
    _In_ ULONG Length)
{
    ...
    ASSERT(StartOffset < PageCount * PAGE_SIZE);
    ASSERT(Length <= PageCount * PAGE_SIZE - StartOffset);

    while (Length != 0)
    {
        ULONG PageIndex = StartOffset >> PAGE_SHIFT;
        ULONG PageOffset = StartOffset & (PAGE_SIZE - 1);
        ULONG ZeroLength = min(Length, PAGE_SIZE - PageOffset);
        KIRQL Irql;
        PVOID Address;

        Address = MiMapPageInHyperSpace(Process, Pages[PageIndex], &Irql);
        if (Address == NULL)
            return STATUS_NO_MEMORY;

        RtlZeroMemory((PUCHAR)Address + PageOffset, ZeroLength);
        MiUnmapPageInHyperSpace(Process, Address, Irql);

        StartOffset += ZeroLength;
        Length -= ZeroLength;
    }
    ...
    _In_ BOOLEAN SetDirty)
{
    ...
    for ( ; RangeStart < RangeEnd; RangeStart += _64K)
    {
        ...
        while (ChunkOffset < ChunkEnd)
        {
            ...
            ULONG ReadPages = BitSet;
            ULONG ReadLength = ReadPages * PAGE_SIZE;
            ...
            if ((ChunkOffset + ReadLength) > ChunkEnd)
            {
                ...
                ReadPages = BYTES_TO_PAGES(ReadLength);
            }
            ...
            BOOLEAN ZeroTail = FALSE;
            ULONG TailOffset = 0;
            FileOffset.QuadPart = Segment->Image.FileOffset + ChunkOffset;

            /* Keep paging I/O page-based, but do not cache bytes beyond VDL. */
            if (ValidDataLength && ((FileOffset.QuadPart + ReadLength) > ValidDataLength->QuadPart))
            {
                ...
                {
                    TailOffset = (ULONG)(ValidDataLength->QuadPart - FileOffset.QuadPart);
                    ASSERT(TailOffset < ReadLength);
                    ZeroTail = TRUE;
                }
            }
            ...
            RtlZeroMemory(Pages, ReadPages * sizeof(PFN_NUMBER));
            for (UINT i = 0; i < ReadPages; i++)
            {
...
Failed:
            ...
            if (ZeroTail)
            {
                Status = MiZeroPageRange(Pages,
                                         ReadPages,
                                         TailOffset,
                                         ReadLength - TailOffset);
                if (!NT_SUCCESS(Status))
                {
                    for (UINT i = 0; i < ReadPages; i++)
                        MmReleasePageMemoryConsumer(MC_USER, Pages[i]);
...
AssignPagesToSegment:
            ...
            IoFreeMdl(Mdl);
            ToReadPageBits >>= ReadPages;
            ChunkOffset += ReadPages * PAGE_SIZE;
        }
    }
    ...
    )
{
    ...
    for (;;)
    {
        ...
        *MappedBase = MiRosFindSystemCacheView(AddressSpace, *ViewSize);
        ...
        MmLockAddressSpace(AddressSpace);
        if (MmIsAddressRangeFree(AddressSpace, *MappedBase, *ViewSize))
        {
            break;
        }
                 ...
                 IN PFILE_OBJECT  FileObject  OPTIONAL)
{
    ...
    if (AllocationAttributes & SEC_IMAGE)
    {
        if (HaveFileObject)
        {
            /* Direct FileObject callers use the legacy data-section path. */
    ...
    _Out_opt_ PIO_STATUS_BLOCK Iosb)
{
    ...
    while (FlushStart.QuadPart < FlushEnd.QuadPart)
    {
        ...
        if (IS_DIRTY_SSE(Entry))
        {
            ...
            ULONG RunPages = 0;
            PFN_NUMBER Pages[MI_MAX_FLUSH_RUN_PAGES];
