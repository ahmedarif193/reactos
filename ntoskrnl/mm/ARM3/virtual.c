                      ...
                      IN ULONG Count)
{
    ...
    //
    // All the PTEs have been dereferenced and made invalid, flush the TLB and
    // release the PFN lock. This must be a cross-processor flush: the freed
    // pages can be reused immediately, so a stale TLB entry on another CPU would
    // point at a recycled physical page (memory corruption). amd64 has no
    // hardware TLB broadcast, so a local KeFlushCurrentTb() is not enough here.
    //
    KeFlushEntireTb(TRUE, TRUE);
    MiReleasePfnLock(OldIrql);
}
