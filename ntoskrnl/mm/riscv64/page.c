 ...
 * PURPOSE:     Legacy page-mapping API on the RISC-V page-table window
 *
 * Derived from the generic i386/page.c that amd64 also uses; only the
 * hardware protection encoding differs (RISC-V R/W/X bits, no PCD/PWT).
 */
...
/* Protection tables live in mmdata.c. */
...
MmGetPageProtect(PEPROCESS Process, PVOID Address)
{
    ...
    else
    {
        ...
        else if (PointerPte->u.Hard.Write)
            Protect = PAGE_READWRITE;
        else
            Protect = PAGE_READONLY;
        if (PointerPte->u.Hard.Execute)
            Protect <<= 4;
    }
...
MmSetPageProtect(PEPROCESS Process, PVOID Address, ULONG flProtect)
{
    ...
    /* A leaf is always accessed; a writable leaf is dirty (no Svadu reliance). */
    TempPte.u.Hard.Accessed = 1;
    TempPte.u.Hard.Dirty = PointerPte->u.Hard.Dirty | TempPte.u.Hard.Write;
