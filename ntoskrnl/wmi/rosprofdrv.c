    ...
    OUT PEPROCESS *ReferencedTarget)
{
    ...
    *ReferencedTarget = NULL;
    ...
    Config->Interval = (PublicConfig->SamplePeriod100ns > MAXULONG) ?
                       MAXULONG : (ULONG)PublicConfig->SamplePeriod100ns;
    if (PublicConfig->Sources & ROSPROF_SOURCE_TIMER)
    {
        ...
        if (PublicConfig->StackFlags & ROSPROF_STACK_KERNEL) Config->Flags |= KPROF_TRACE_FLAG_CALLCHAIN;
    }
    ...
    IN OUT PROSPROF_FILE_CONTEXT Context)
{
    ...
    /* ProfileTime is one global HAL source. CONFIGURE establishes the
     * normalized target and START reapplies it; a later profile caller may
     * change the global source, so sample weights carry the live value. */
    KeSetIntervalProfile(Interval, ProfileTime);
    Interval = KeQueryIntervalProfile(ProfileTime);
    if (!Interval) return STATUS_NOT_SUPPORTED;
    ...
    OUT PROSPROF_SESSION_INFO_V1 SessionInfo)
{
    ...
    SessionInfo->MaximumStackDepth = 1;
    if (Context->InternalConfig.Flags & KPROF_TRACE_FLAG_CALLCHAIN) SessionInfo->MaximumStackDepth = KPROF_TRACE_MAX_FRAMES;
    ...
    OUT PULONG_PTR Information)
{
    ...
    Status = RosprofNormalizeTimerInterval(Context);
    if (!NT_SUCCESS(Status))
    {
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        if (NT_SUCCESS(Status) && Context->Started && (Stats.State == 1))
        {
            ...
            if (Context->AcceptedRecordTypes &
                ROSPROF_RECORD_MASK(ROSPROF_RECORD_SESSION))
            {
                KprofTraceEmitSessionRecord(Context->Session, ROSPROF_SESSION_EVENT_END, Command->Reason ? Command->Reason : ROSPROF_REASON_REQUESTED, STATUS_SUCCESS, Context->AcceptedSources, Context->ConfigGeneration);
            }
        }
        Status = KprofTraceStopSession(Context->Session);
        if (NT_SUCCESS(Status)) Context->Started = FALSE;
    }
    KeReleaseGuardedMutex(&Context->Lock);
    return Status;
}
    ...
    OUT PULONG_PTR Information)
{
    ...
    PublicStatus->FinalStatus = STATUS_SUCCESS;
